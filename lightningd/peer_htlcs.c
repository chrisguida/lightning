...
static bool state_update_ok(struct channel *channel, enum htlc_state oldstate,
			    enum htlc_state newstate, u64 htlc_id,
			    const char *dir)
{
	...
	if (newstate != expected) {
		channel_internal_error(
		    channel, "HTLC %s %" PRIu64 " invalid update %s->%s", dir,
		    htlc_id, htlc_state_name(oldstate),
		    htlc_state_name(newstate));
		return false;
	}

	log_debug(channel->log, "HTLC %s %" PRIu64 " %s->%s", dir, htlc_id,
		  htlc_state_name(oldstate), htlc_state_name(newstate));
	return true;
}

static bool htlc_in_update_state(struct channel *channel, struct htlc_in *hin,
				 enum htlc_state newstate)
{
	if (!state_update_ok(channel, hin->hstate, newstate, hin->key.id, "in"))
		return false;

	wallet_htlc_update(channel->peer->ld->wallet, hin->dbid, newstate,
			   hin->preimage,
			   max_unsigned(channel->next_index[LOCAL],
					channel->next_index[REMOTE]),
			   hin->badonion, hin->failonion, NULL, hin->we_filled);
				  ...
				  enum htlc_state newstate)
{
	...
	wallet_htlc_update(channel->peer->ld->wallet, hout->dbid, newstate,
			   ...
			   max_unsigned(channel->next_index[LOCAL],
					channel->next_index[REMOTE]),
			   0, hout->failonion, hout->failmsg, &we_filled);
...
static bool blind_error_return(const struct htlc_in *hin)
{
	...
	if (hin->payload && hin->payload->blinding && !hin->payload->final)
		return true;
					  ...
					  const struct onionreply *failonion)
{
	...
	if (hin->payload && hin->payload->blinding) {
		...
		sha256(&sha, hin->onion_routing_packet,
		       sizeof(hin->onion_routing_packet));
		failonion = create_onionreply(
		    tmpctx, hin->shared_secret,
			 ...
			 const struct onionreply *failonion TAKES)
{
	...
	bool we_filled = false;
	wallet_htlc_update(hin->key.channel->peer->ld->wallet, hin->dbid,
			   hin->hstate, hin->preimage,
			   max_unsigned(hin->key.channel->next_index[LOCAL],
					hin->key.channel->next_index[REMOTE]),
			   hin->badonion, hin->failonion, NULL, &we_filled);
...
void local_fail_in_htlc(struct htlc_in *hin, const u8 *failmsg TAKES)
{
	struct onionreply *failonion =
	    create_onionreply(NULL, hin->shared_secret, failmsg);
...
const u8 *failmsg_incorrect_or_unknown_(const tal_t *ctx, struct lightningd *ld,
					const struct htlc_in *hin,
					const char *file, int line)
{
	...
	return towire_incorrect_or_unknown_payment_details(
	    ctx, hin->msat, get_block_height(ld->topology));
}
...
static void fail_out_htlc(struct htlc_out *hout, const char *localfail)
{
	...
	if (hout->am_origin) {
		...
		/* Otherwise, we need to onionize this local error. */
			...
			failonion = create_onionreply(
			    hout, hout->in->shared_secret, hout->failmsg);
 ...
 * ...
 *        - incoming `amount_msat` - `fee` < `amt_to_forward` (where `fee` is
 * the advertised fee as described in [BOLT #7](07-routing-gossip.md#htlc-fees))
 */
static bool check_fwd_amount(struct htlc_in *hin,
			     struct amount_msat amt_to_forward,
			     struct amount_msat amt_in_htlc, u32 feerate_base,
			     u32 feerate_ppm)
{
	struct amount_msat fee;
	struct amount_msat fwd;

	if (!amount_msat_fee(&fee, amt_to_forward, feerate_base, feerate_ppm)) {
 ...
 */
static bool check_cltv(struct htlc_in *hin, u32 cltv_expiry,
		       u32 outgoing_cltv_value, u32 delta)
{
	if (delta < cltv_expiry && cltv_expiry - delta >= outgoing_cltv_value)
		return true;
	log_debug(hin->key.channel->log,
		  "HTLC %" PRIu64 " incorrect CLTV:"
...
void fulfill_htlc(struct htlc_in *hin, const struct preimage *preimage)
{
	...
	enum htlc_state final_state = channel->our_config.is_eltoo
					  ? SENT_ADD_ACK
	...
	wallet_channel_stats_incr_in_fulfilled(wallet, channel->dbid,
					       hin->msat);
...
static void
handle_localpay(struct htlc_in *hin, struct amount_msat amt_to_forward,
		u32 outgoing_cltv_value, struct amount_msat total_msat,
		const struct secret *payment_secret, const u8 *payment_metadata)
{
	...
	/* BOLT #4:
	 *   - If it is the final node:
	 *     - MUST treat `total_msat` as if it were equal to `amt_to_forward`
	...
	if (!check_cltv(hin, hin->cltv_expiry, outgoing_cltv_value, 0)) {
		 ...
		 *    * [`u32`:`cltv_expiry`]
		 *
		 * The CLTV expiry in the HTLC is less than the value in the
		 * onion.
		 */
		failmsg =
		    towire_final_incorrect_cltv_expiry(NULL, hin->cltv_expiry);
		goto fail;
	}

	/* BOLT #4:
	 *
	 *   incoming `cltv_expiry` < `current_block_height` +
	 * `min_final_cltv_expiry_delta`.	 */
	if (get_block_height(ld->topology) + ld->config.cltv_final >
	    hin->cltv_expiry) {
		log_debug(hin->key.channel->log,
			  "Expiry cltv too soon %u < %u + %u", hin->cltv_expiry,
	...
	if (payment_metadata) {
		 ...
		 *    * [`u16`:`offset`]
		 *
		 * The decrypted onion per-hop payload was not understood by the
		 * processing node or is incomplete. If the failure can be
		 * narrowed down to a specific tlv type in the payload, the
		 * erring node may include that `type` and its byte `offset` in
		 * the decrypted byte stream.
		 */
		failmsg = towire_invalid_onion_payload(
		    NULL, TLV_PAYLOAD_PAYMENT_METADATA,
		    /* FIXME: offset? */ 0);
		goto fail;
	}
...
static void destroy_hout_subd_died(struct htlc_out *hout)
{
	...
	bool have_tx = db_in_transaction(db);
	log_debug(hout->key.channel->log,
		  "Failing HTLC %" PRIu64 " due to peer death", hout->key.id);

	hout->failmsg = towire_temporary_channel_failure(
	    hout, get_channel_update(hout->key.channel));
 ...
 * failed immediately. */
static void rcvd_htlc_reply(struct subd *subd, const u8 *msg,
			    const int *fds UNUSED, struct htlc_out *hout)
{
	u8 *failmsg;
	char *failurestr;
	struct lightningd *ld = subd->ld;

	if (!fromwire_channeld_offer_htlc_reply(msg, msg, &hout->key.id,
						&failmsg, &failurestr)) {
	...
	if (tal_count(failmsg)) {
		...
		if (hout->am_origin) {
			...
			wallet_forwarded_payment_add(
			    ld->wallet, hout->in, FORWARD_STYLE_TLV,
			    channel_scid_or_local_alias(hout->key.channel),
			    NULL, FORWARD_LOCAL_FAILED,
	...
	if (find_htlc_out(subd->ld->htlcs_out, hout->key.channel,
			  hout->key.id) ||
...
const u8 *send_htlc_out(const tal_t *ctx, struct channel *out,
			struct amount_msat amount, u32 cltv,
			struct amount_msat final_msat,
			const struct sha256 *payment_hash,
			const struct pubkey *blinding, u64 partid, u64 groupid,
			const u8 *onion_routing_packet, struct htlc_in *in,
			struct htlc_out **houtp)
{
	...
	*houtp = NULL;
	...
	if (!out->owner) {
		log_info(out->log, "Attempt to send HTLC but unowned (%s)",
			 channel_state_name(out));
		return towire_temporary_channel_failure(
		    ctx, get_channel_update(out));
	}
	...
	*houtp = new_htlc_out(out->owner, out, amount, cltv, payment_hash,
			      onion_routing_packet, blinding, in == NULL,
			      final_msat, partid, groupid, in);
	...
	if (!out->peer->ld->dev_no_htlc_timeout) {
		(*houtp)->timeout =
		    new_reltimer(out->peer->ld->timers, *houtp,
				 time_from_sec(30), htlc_offer_timeout, *houtp);
	}
...
static void
forward_htlc(struct htlc_in *hin, u32 cltv_expiry,
	     ...
	     const struct pubkey *next_blinding)
{
	...
	if (!next || !channel_state_can_add_htlc(next->state)) {
		...
		wallet_forwarded_payment_add(
		    hin->key.channel->peer->ld->wallet, hin, FORWARD_STYLE_TLV,
		    forward_scid, NULL, FORWARD_LOCAL_FAILED,
	...
	if (!check_fwd_amount(hin, amt_to_forward, hin->msat,
			      next->feerate_base, next->feerate_ppm)) {
		/* BOLT #7:
		 * - If it creates a new `channel_update` with updated channel
		 * parameters:
		 *    - SHOULD keep accepting the previous channel parameters
		 * for 10 minutes
		 */
		if (!time_before(time_now(), next->old_feerate_timeout) ||
			...
			failmsg = towire_fee_insufficient(
			    tmpctx, hin->msat, get_channel_update(next));
			goto fail;
		}
	...
	if (amount_msat_greater(amt_to_forward, next->htlc_maximum_msat) ||
		...
		if (!time_before(time_now(), next->old_feerate_timeout) ||
		    amount_msat_less(amt_to_forward,
				     next->old_htlc_minimum_msat) ||
		    amount_msat_greater(amt_to_forward,
					next->old_htlc_maximum_msat)) {
			failmsg = towire_temporary_channel_failure(
			    tmpctx, get_channel_update(next));
			goto fail;
		}
		log_info(hin->key.channel->log,
	...
	if (!check_cltv(hin, cltv_expiry, outgoing_cltv_value,
		...
		failmsg = towire_incorrect_cltv_expiry(
		    tmpctx, cltv_expiry, get_channel_update(next));
		goto fail;
	}
	 ...
	 *     `cltv_expiry`.
	 */
	/* In our case, G = 1, so we need to expire it one after it's
	 * expiration. But never offer an expired HTLC; that's dumb. */
	if (get_block_height(ld->topology) >= outgoing_cltv_value) {
		log_debug(hin->key.channel->log,
			  "Expiry cltv %u too close to current %u",
			  outgoing_cltv_value, get_block_height(ld->topology));
		failmsg =
		    towire_expiry_too_soon(tmpctx, get_channel_update(next));
		goto fail;
	}
	 ...
	 */
	if (get_block_height(ld->topology) + ld->config.locktime_max <
	    outgoing_cltv_value) {
		log_debug(hin->key.channel->log,
	...
	failmsg = send_htlc_out(
	    tmpctx, next, amt_to_forward, outgoing_cltv_value, AMOUNT_MSAT(0),
	    &hin->payment_hash, next_blinding, 0 /* partid */, 0 /* groupid */,
	    next_onion, hin, &hout);
	if (!failmsg)
		return;
...
fail:
	...
	wallet_forwarded_payment_add(ld->wallet, hin, FORWARD_STYLE_TLV,
				     forward_scid, hout, FORWARD_LOCAL_FAILED,
...
static u8 *convert_failcode(const tal_t *ctx, struct lightningd *ld,
 ...
 */
static bool
htlc_accepted_hook_deserialize(struct htlc_accepted_hook_payload *request,
			       const char *buffer, const jsmntok_t *toks)
{
	...
	if (payloadtok) {
		...
		request->payload = onion_decode(
		    request,
		    feature_offered(ld->our_features->bits[INIT_FEATURE],
				    OPT_ROUTE_BLINDING),
		    rs, hin->blinding, ld->accept_extra_tlv_types, hin->msat,
		    hin->cltv_expiry, &request->failtlvtype,
		    &request->failtlvpos);
	}
	...
	if (json_tok_streq(buffer, resulttok, "fail")) {
		...
		if (failoniontok) {
			failonion =
			    json_tok_bin_from_hex(tmpctx, buffer, failoniontok);
			if (!failonion)
			...
			if (failmsgtok)
				log_broken(ld->log,
					   "Both 'failure_onion' and"
					   "'failure_message' provided."
					   " Ignoring 'failure_message'.");

			fail_in_htlc(hin,
				     take(new_onionreply(NULL, failonion)));
			return false;
		}
		if (failmsgtok) {
			failmsg =
			    json_tok_bin_from_hex(NULL, buffer, failmsgtok);
			if (!failmsg)
					 ...
					 struct plugin *plugin)
{
	...
	s32 expiry = hin->cltv_expiry,
	    blockheight = p->ld->topology->tip->height;
	...
	if (p->payload) {
		...
		json_add_u32(s, "outgoing_cltv_value",
	...
	if (!request->payload) {
		...
		local_fail_in_htlc(
		    hin, take(towire_invalid_onion_payload(
			     NULL, request->failtlvtype, request->failtlvpos)));
	} else if (rs->nextcase == ONION_FORWARD) {
		log_debug(channel->log, "Forwarding HTLC");
		forward_htlc(
		    hin, hin->cltv_expiry, request->payload->amt_to_forward,
		    request->payload->outgoing_cltv,
		    request->payload->forward_channel, request->fwd_channel_id,
		    serialize_onionpacket(tmpctx, rs->next),
		    request->next_blinding);
	} else {
		log_debug(channel->log, "Handling HTLC locally");
		handle_localpay(hin, request->payload->amt_to_forward,
				request->payload->outgoing_cltv,
				...
				struct secret *ss)
{
	...
	if (blinding) {
		 ...
		 * our real key */
		if (secp256k1_ec_pubkey_tweak_mul(secp256k1_ctx, &point.pubkey,
						  hmac.data) != 1) {
			return false;
		}
	}
...
REGISTER_PLUGIN_HOOK(htlc_accepted, htlc_accepted_hook_deserialize,
		     htlc_accepted_hook_final, htlc_accepted_hook_serialize,
		     struct htlc_accepted_hook_payload *);

/* Figures out how to fwd, allocating return off hp */
static struct channel_id *
calc_forwarding_channel(struct lightningd *ld,
			struct htlc_accepted_hook_payload *hp)
{
	...
	if (p->forward_channel) {
		...
		if (!c) {
			log_unusual(
			    hp->channel->log, "No peer channel with scid=%s",
			    type_to_string(tmpctx, struct short_channel_id,
					   p->forward_channel));
			return NULL;
		}
	...
	if (!c) {
		if (!best)
			return NULL;
		log_debug(hp->channel->log, "Chose channel %s for peer %s",
			  type_to_string(tmpctx, struct short_channel_id,
 ...
 */
static bool peer_accepted_htlc(const tal_t *ctx, struct channel *channel,
			       u64 id, bool replay, enum onion_wire *badonion,
			       u8 **failmsg)
{
	struct htlc_in *hin;
	...
	const bool opt_blinding = feature_offered(
	    ld->our_features->bits[INIT_FEATURE], OPT_ROUTE_BLINDING);

	/* FIXME Need better switch/field to check if eltoo... */
	/* FIXME 2: Maybe we *do* want to jump to same state, then adjust
	...
	*badonion = 0;
	...
	if (!hin) {
		channel_internal_error(
		    channel, "peer_got_revoke unknown htlc %" PRIu64, id);
		*failmsg = towire_temporary_node_failure(ctx);
		goto fail;
	}

	if (hin->fail_immediate &&
	    htlc_in_update_state(channel, hin, new_state)) {
	...
	if (channel->peer->dev_ignore_htlcs) {
		log_debug(channel->log,
			  "their htlc %" PRIu64 " dev_ignore_htlcs", id);
		return true;
	}
	...
	op = parse_onionpacket(tmpctx, hin->onion_routing_packet,
			       sizeof(hin->onion_routing_packet), badonion);
	if (!op) {
		log_debug(channel->log,
			  "Rejecting their htlc %" PRIu64
	...
	hook_payload->payload =
	    onion_decode(hook_payload,
			 feature_offered(ld->our_features->bits[INIT_FEATURE],
					 OPT_ROUTE_BLINDING),
			 rs, hin->blinding, ld->accept_extra_tlv_types,
			 hin->msat, hin->cltv_expiry,
			 &hook_payload->failtlvtype, &hook_payload->failtlvpos);
	hook_payload->ld = ld;
	hook_payload->hin = hin;
	hook_payload->channel = channel;
	hook_payload->next_onion =
	    serialize_onionpacket(hook_payload, rs->next);

	/* We could have blinding from hin or from inside onion. */
	if (opt_blinding && hook_payload->payload &&
	    hook_payload->payload->blinding) {
		struct sha256 sha;
				 ...
				 const struct preimage *preimage)
{
	...
	wallet_htlc_update(ld->wallet, hout->dbid, hout->hstate, hout->preimage,
			   max_unsigned(channel->next_index[LOCAL],
					channel->next_index[REMOTE]),
			   0, hout->failonion, hout->failmsg, &we_filled);
	/* Update channel stats */
	wallet_channel_stats_incr_out_fulfilled(ld->wallet, channel->dbid,
						hout->msat);

	if (hout->am_origin)
		payment_succeeded(ld, &hout->payment_hash, hout->partid,
				  hout->groupid, preimage);
	else if (hout->in) {
		...
		if (hout->in->failonion) {
			...
			log_unusual(
			    channel->log,
			    "FUNDS LOSS of %s: peer took funds onchain before "
			    "we could time out the HTLC, but we abandoned "
			    "incoming HTLC to save the incoming channel",
				    ...
				    const struct fulfilled_htlc *fulfilled)
{
	...
	if (!hout) {
		channel_internal_error(
		    channel, "fulfilled_our_htlc unknown htlc %" PRIu64,
		    fulfilled->id);
		return false;
	}
				 ...
				 const struct failed_htlc *failed)
{
	...
	if (hout->in)
		wallet_forwarded_payment_add(
		    ld->wallet, hout->in, FORWARD_STYLE_TLV,
		    channel_scid_or_local_alias(channel), hout, FORWARD_FAILED,
		    hout->failmsg ? fromwire_peektype(hout->failmsg) : 0);
...
static void check_already_failed(const struct channel *channel,
				 struct htlc_out *hout)
{
	htlc_out_check(hout, __func__);
	...
	if (hout->in->badonion != 0 || hout->in->failonion ||
	    hout->in->preimage)
		return;
	...
	log_broken(channel->log,
		   "HTLC id %" PRIu64
		   " already complete, but ->in not resolved!"
		   " failonion = %s, failmsg = %s, preimage = %s",
		   hout->key.id,
		   hout->failonion ? tal_hex(tmpctx, hout->failonion->contents)
				   : "(null)",
		   hout->failmsg ? tal_hex(tmpctx, hout->failmsg) : "(null)",
		   hout->preimage
		       ? type_to_string(tmpctx, struct preimage, hout->preimage)
		       : "(null)");

	if (hout->preimage) {
		/* Log on both ours and theirs! */
		log_broken(channel->log,
			   "MISSING incoming success for %" PRIu64
			   ": succeeding incoming now",
			   hout->key.id);
		log_broken(hout->in->key.channel->log,
			   "MISSED incoming success for %" PRIu64
			   ": succeeding now",
			   hout->in->key.id);
		fulfill_htlc(hout->in, hout->preimage);
	} else {
		log_broken(channel->log,
			   "MISSING incoming fail for %" PRIu64
			   ": failing incoming now",
			   hout->key.id);
		log_broken(hout->in->key.channel->log,
			   "MISSED incoming fail for %" PRIu64 ": failing now",
			   hout->in->key.id);
		local_fail_in_htlc(
		    hout->in, take(towire_permanent_channel_failure(NULL)));
	}
}
	...
	for (hin = htlc_in_map_first(ld->htlcs_in, &ini); hin;
		...
		if (hin->badonion) {
			log_broken(hin->key.channel->log,
				   "htlc %" PRIu64
				   " already failed with badonion",
				   hin->key.id);
		} else if (hin->preimage) {
			log_broken(hin->key.channel->log,
				   "htlc %" PRIu64
				   " already succeeded with preimage",
				   hin->key.id);
		} else if (hin->failonion) {
			log_broken(hin->key.channel->log,
				   "htlc %" PRIu64
				   " already failed with failonion %s",
				   hin->key.id,
				   tal_hex(tmpctx, hin->failonion->contents));
		} else {
			log_broken(hin->key.channel->log,
				   "htlc %" PRIu64
				   " has matching hash: failing",
				   hin->key.id);
			local_fail_in_htlc(
			    hin, take(towire_permanent_channel_failure(NULL)));
		}
	}
}

void onchain_failed_our_htlc(const struct channel *channel,
			     const struct htlc_stub *htlc, const char *why,
			     bool should_exist)
{
	struct lightningd *ld = channel->peer->ld;
	struct htlc_out *hout;
	...
	if (!hout) {
		...
		if (should_exist)
			log_broken(channel->log,
				   "HTLC id %" PRIu64 " not found!", htlc->id);
		/* Immediate corruption sanity check if this happens */
		htable_check(&ld->htlcs_out->raw,
			     "onchain_failed_our_htlc out");
	...
	if (hout->failonion && hout->in && hout->in->badonion == 0 &&
	    !hout->in->failonion && !hout->in->preimage) {
		log_debug(channel->log,
			  "HTLC out %" PRIu64 " can now fail HTLC upstream!",
			  htlc->id);
	...
	if (hout->failonion || hout->failmsg || hout->preimage) {
		log_debug(channel->log,
			  "HTLC id %" PRIu64
			  " failonion = %p, failmsg = %p, preimage = %p",
			  htlc->id, hout->failonion, hout->failmsg,
			  hout->preimage);
	...
	if (hout->am_origin) {
		log_debug(channel->log, "HTLC id %" PRIu64 " am origin",
			  htlc->id);
		assert(why != NULL);
		char *localfail = tal_fmt(
		    channel, "%s: %s",
		    onion_wire_name(WIRE_PERMANENT_CHANNEL_FAILURE), why);
		payment_failed(ld, hout, localfail);
		tal_free(localfail);
	} else if (hout->in) {
		...
		wallet_forwarded_payment_add(
		    hout->key.channel->peer->ld->wallet, hout->in,
		    FORWARD_STYLE_TLV, channel_scid_or_local_alias(channel),
		    hout, FORWARD_LOCAL_FAILED,
		    hout->failmsg ? fromwire_peektype(hout->failmsg) : 0);
	} else {
...
static void remove_htlc_in(struct channel *channel, struct htlc_in *hin)
{
	...
	if (hin->preimage) {
		...
		if (!amount_msat_add(&channel->our_msat, channel->our_msat,
				     hin->msat)) {
			channel_internal_error(
			    channel, "Overflow our_msat %s + HTLC %s",
			    type_to_string(tmpctx, struct amount_msat,
					   &channel->our_msat),
			    type_to_string(tmpctx, struct amount_msat,
					   &hin->msat));
...
static void remove_htlc_out(struct channel *channel, struct htlc_out *hout)
{
	...
	log_debug(channel->log, "Removing out HTLC %" PRIu64 " state %s %s",
		  ...
		  : hout->failmsg
		      ? onion_wire_name(fromwire_peektype(hout->failmsg))
		      : "REMOTEFAIL");
	...
	if (!hout->preimage) {
		...
		if (!amount_msat_sub(&channel->our_msat, channel->our_msat,
				     hout->msat)) {
			channel_internal_error(
			    channel, "Underflow our_msat %s - HTLC %s",
			    type_to_string(tmpctx, struct amount_msat,
					   &channel->our_msat),
			    type_to_string(tmpctx, struct amount_msat,
					   &hout->msat));
		...
		log_debug(channel->log, "Balance %s -> %s",
			  ...
			  type_to_string(tmpctx, struct amount_msat,
					 &channel->our_msat));
		if (amount_msat_less(channel->our_msat,
...
static bool update_in_htlc(struct channel *channel, u64 id,
			   enum htlc_state newstate)
{
	struct htlc_in *hin;
	struct lightningd *ld = channel->peer->ld;
	...
	hin = find_htlc_in(ld->htlcs_in, channel, id);
	if (!hin) {
		channel_internal_error(channel, "Can't find in HTLC %" PRIu64,
				       id);
		return false;
	}
...
static bool update_out_htlc(struct channel *channel, u64 id,
			    enum htlc_state newstate)
{
	struct lightningd *ld = channel->peer->ld;
	struct htlc_out *hout;
	...
	if (!hout) {
		channel_internal_error(channel, "Can't find out HTLC %" PRIu64,
				       id);
		return false;
	}
	...
	if (!hout->dbid) {
		...
		/* Update channel stats */
		wallet_channel_stats_incr_out_offered(ld->wallet, channel->dbid,
						      hout->msat);
				...
				const struct bitcoin_tx *tx)
{
	...
	if (tx->wtx->num_outputs == 0) {
		channel_internal_error(
		    channel, "channel_got_commitsig: zero output tx! %s",
...
static bool peer_save_updatesig_received(
    struct channel *channel, u64 update_num, struct bitcoin_tx *update_tx,
    struct bitcoin_tx *settle_tx, struct partial_sig *their_psig,
    struct partial_sig *our_psig, struct musig_session *session)
{
...
static bool
peer_save_commitsig_received(struct channel *channel, u64 commitnum,
...
static bool peer_save_updatesig_sent(
    struct channel *channel, u64 update_num, struct partial_sig *our_psig,
    struct musig_session *session, struct bitcoin_tx *committed_update_tx TAKES,
    struct bitcoin_tx *committed_settle_tx TAKES)
{
	...
	channel->eltoo_keyset.committed_update_tx =
	    tal_steal(channel, committed_update_tx);
	channel->eltoo_keyset.committed_settle_tx =
	    tal_steal(channel, committed_settle_tx);

	wallet_channel_save(ld->wallet, channel);
	return true;
}

static bool peer_save_updatesig_resent(struct channel *channel, u64 update_num,
...
void peer_got_ack(struct channel *channel, const u8 *msg)
{
	...
	if (!fromwire_channeld_got_ack(msg, msg, &update_num, &changed,
				       &their_psig, &our_psig, &session)) {
		channel_internal_error(channel,
				       "bad channel_sending_updatesig_ack %s",
				       tal_hex(channel, msg));
		return;
	}

	log_debug(channel->log, "got ack %" PRIu64 ": %zu changed", update_num,
		  tal_count(changed));
	...
	for (i = 0; i < tal_count(changed); i++) {
		if (!changed_htlc(channel, changed + i)) {
			channel_internal_error(
			    channel, "channel_got_ack: update failed");
			return;
		}
	}
	...
	channel->eltoo_keyset.complete_update_tx =
	    tal_steal(channel, channel->eltoo_keyset.committed_update_tx);
	channel->eltoo_keyset.committed_update_tx = NULL;
	channel->eltoo_keyset.complete_settle_tx =
	    tal_steal(channel, channel->eltoo_keyset.committed_settle_tx);
	...
	subd_send_msg(channel->owner, take(towire_channeld_got_ack_reply(msg)));
}
...
void peer_resending_updatesig(struct channel *channel, const u8 *msg)
{
	...
	if (!fromwire_channeld_resending_updatesig(
		msg, &update_num, &our_update_psig, &session)) {
		channel_internal_error(channel,
				       "bad channel_resending_updatesig %s",
				       tal_hex(channel, msg));
		return;
	}

	/* Add newest state to channel, then save to db */
	if (!peer_save_updatesig_resent(channel, update_num, &our_update_psig,
					&session))
		return;
...
void peer_sending_updatesig(struct channel *channel, const u8 *msg)
{
	...
	if (!fromwire_channeld_sending_updatesig(
		msg, msg, &update_num, &changed_htlcs, &our_update_psig,
		&session, &committed_update_tx, &committed_settle_tx)) {
		channel_internal_error(channel,
				       "bad channel_sending_updatesig %s",
				       tal_hex(channel, msg));
		return;
	}
	...
	/* Add newest state to channel, then save to db */
	if (!peer_save_updatesig_sent(channel, update_num, &our_update_psig,
				      &session, committed_update_tx,
				      committed_settle_tx))
		return;
...
void peer_sending_commitsig(struct channel *channel, const u8 *msg)
{
	...
	if (!fromwire_channeld_sending_commitsig(
		msg, msg, &commitnum, &pbase, &fee_states, &blockheight_states,
		&changed_htlcs) ||
	    !fee_states_valid(fee_states, channel->opener) ||
	    !height_states_valid(blockheight_states, channel->opener)) {
		channel_internal_error(channel,
				       "bad channel_sending_commitsig %s",
				       tal_hex(channel, msg));
		return;
	}
	...
	adjust_channel_feerate_bounds(
	    channel, get_feerate(fee_states, channel->opener, REMOTE));
				     ...
				     const struct added_htlc *added)
{
	...
	if (amount_msat_eq(added->amount, AMOUNT_MSAT(0)) ||
		...
		channel_internal_error(
		    ...
		    " but minimum is %s",
		    type_to_string(tmpctx, struct amount_msat, &added->amount),
		    type_to_string(tmpctx, struct amount_msat,
	...
	op = parse_onionpacket(tmpctx, added->onion_routing_packet,
			       sizeof(added->onion_routing_packet), &failcode);
	if (op) {
		if (!ecdh_maybe_blinding(&op->ephemeralkey, added->blinding,
					 &shared_secret)) {
			log_debug(channel->log,
				  "htlc %" PRIu64 ": can't tweak pubkey",
				  added->id);
			return false;
		}
	}
	...
	hin = new_htlc_in(channel, channel, added->id, added->amount,
			  ...
			  op ? &shared_secret : NULL, added->blinding,
			  added->onion_routing_packet, added->fail_immediate);
			     ...
			     struct changed_htlc *changed)
{
	...
	for (i = 0; i < tal_count(fulfilled); i++) {
		if (!update_out_htlc(channel, fulfilled[i].id, SENT_REMOVE_ACK))
			return false;
	}
	/* FIXE no idea if this stuff is required
	    for (i = 0; i < tal_count(failed); i++) {
		    if (!update_out_htlc(channel, failed[i]->id,
	   SENT_REMOVE_REVOCATION)) return false;
	    }
	    for (i = 0; i < tal_count(changed); i++) {
...
void peer_got_updatesig(struct channel *channel, const u8 *msg)
{
	...
	if (!fromwire_channeld_got_updatesig(
		msg, msg, &update_num, &our_psig, &their_psig, &session, &added,
		&fulfilled, &failed, &changed, &update_tx, &settle_tx)) {
		channel_internal_error(channel,
	...
	log_debug(channel->log,
		  ...
		  update_num, tal_count(added), tal_count(fulfilled),
	...
	/* Since we're about to send ACK(and fulfilled a second ago), bump state
	 * again and delete complete htlcs. */
	if (!peer_sending_ack(channel, fulfilled, failed, changed))
		return;

	/* Stores fully signed transactions, and partial sigs for
	 * reestablishment! */
	if (!peer_save_updatesig_received(channel, update_num, update_tx,
					  settle_tx, &their_psig, &our_psig,
					  &session))
		return;
	...
	for (i = 0; i < tal_count(added); i++) {
		...
		peer_accepted_htlc(failmsgs, channel, added[i].id, false,
				   &badonions[i], &failmsgs[i]);
	}

	if (update_num >= (1ULL << 48)) {
		channel_internal_error(
		    channel, "got_ack: too many txs %" PRIu64, update_num);
		return;
	}
	...
	for (i = 0; i < tal_count(changed); i++) {
		...
		if (badonions[i]) {
			hin =
			    find_htlc_in(&ld->htlcs_in, channel, changed[i].id);
		...
		// in fact, now we don't know if this htlc is a forward or
		// localpay!
		wallet_forwarded_payment_add(
		    ld->wallet, hin, FORWARD_STYLE_UNKNOWN, NULL, NULL,
	...
	subd_send_msg(channel->owner,
		      take(towire_channeld_got_updatesig_reply(msg)));
}
...
void peer_got_commitsig(struct channel *channel, const u8 *msg)
{
	...
	if (!fromwire_channeld_got_commitsig(
		msg, msg, &commitnum, &fee_states, &blockheight_states,
		&commit_sig, &htlc_sigs, &added, &fulfilled, &failed, &changed,
		&tx, &inflight_commit_sigs) ||
	    !fee_states_valid(fee_states, channel->opener) ||
	    !height_states_valid(blockheight_states, channel->opener)) {
		channel_internal_error(channel,
				       "bad fromwire_channeld_got_commitsig %s",
	...
	log_debug(channel->log,
		  ...
		  tal_count(added), tal_count(fulfilled), tal_count(failed),
		  tal_count(changed), tal_count(inflight_commit_sigs));
	...
	channel->fee_states = tal_steal(channel, fee_states);
	adjust_channel_feerate_bounds(
	    channel, get_feerate(fee_states, channel->opener, LOCAL));
	tal_free(channel->blockheight_states);
	channel->blockheight_states = tal_steal(channel, blockheight_states);

	/* Since we're about to send revoke, bump state again. */
	if (!peer_sending_revocation(channel, added, fulfilled, failed,
				     changed))
		return;
...
void peer_got_revoke(struct channel *channel, const u8 *msg)
{
	...
	if (!fromwire_channeld_got_revoke(
		msg, msg, &revokenum, &per_commitment_secret,
		&next_per_commitment_point, &fee_states, &blockheight_states,
		&changed, &pbase, &penalty_tx) ||
	    !fee_states_valid(fee_states, channel->opener) ||
	    !height_states_valid(blockheight_states, channel->opener)) {
		channel_internal_error(channel,
				       "bad fromwire_channeld_got_revoke %s",
				       tal_hex(channel, msg));
		return;
	}

	log_debug(channel->log, "got revoke %" PRIu64 ": %zu changed",
	...
	for (i = 0; i < tal_count(changed); i++) {
		...
		if (changed[i].newstate == RCVD_ADD_ACK_REVOCATION) {
			peer_accepted_htlc(failmsgs, channel, changed[i].id,
					   false, &badonions[i], &failmsgs[i]);
		} else {
			if (!changed_htlc(channel, &changed[i])) {
				channel_internal_error(
				    channel, "got_revoke: update failed");
				return;
			}
		}
	}
	...
	if (revokenum >= (1ULL << 48)) {
		channel_internal_error(
		    channel, "got_revoke: too many txs %" PRIu64, revokenum);
		return;
	}

	if (revokenum != revocations_received(&channel->their_shachain.chain)) {
		channel_internal_error(
		    channel, "got_revoke: expected %" PRIu64 " got %" PRIu64,
	 ...
	 *     in [BOLT
	 * #3](03-transactions.md#per-commitment-secret-requirements):
	 *     - MAY send a `warning` and close the connection, or send an
	 *       `error` and fail the channel.
	 */
	if (!wallet_shachain_add_hash(ld->wallet, &channel->their_shachain,
				      shachain_index(revokenum),
				      &per_commitment_secret)) {
		channel_fail_permanent(
		    channel, REASON_PROTOCOL,
		    "Bad per_commitment_secret %s for %" PRIu64,
		    type_to_string(msg, struct secret, &per_commitment_secret),
		    revokenum);
		return;
	}
					...
					const struct channel *channel)
{
	...
	for (hin = htlc_in_map_first(ld->htlcs_in, &ini); hin;
		...
		if (hin->badonion)
			f = take(
			    mk_failed_htlc_badonion(NULL, hin, hin->badonion));
		else if (hin->failonion)
			f = take(mk_failed_htlc(NULL, hin, hin->failonion));
		else
			f = NULL;

		existing = new_existing_htlc(
		    htlcs, hin->key.id, hin->hstate, hin->msat,
		    &hin->payment_hash, hin->cltv_expiry,
		    hin->onion_routing_packet, hin->blinding, hin->preimage, f);
 ...
 * normally need it). */
static void consider_failing_incoming(struct lightningd *ld, u32 height,
				      struct htlc_out *hout)
{
...
void htlcs_notify_new_block(struct lightningd *ld, u32 height)
{
	...
	do {
		...
		for (hout = htlc_out_map_first(ld->htlcs_out, &outi); hout;
			...
			if (!channel_state_can_add_htlc(
				hout->key.channel->state)) {
			...
			channel_fail_permanent(
			    hout->key.channel, REASON_PROTOCOL,
	 ...
	 *     - MUST estimate a fulfillment deadline.
	 *...
	 *   - if an HTLC it has fulfilled is in either node's current
	 *commitment transaction, AND is past this fulfillment deadline:
	...
	do {
		...
		for (hin = htlc_in_map_first(ld->htlcs_in, &ini); hin;
			...
			channel_fail_permanent(channel, REASON_PROTOCOL,
...
static void fixup_hout(struct lightningd *ld, struct htlc_out *hout)
{
	...
	if (hout->in && hout->in->preimage) {
		hout->preimage =
		    tal_dup(hout, struct preimage, hout->in->preimage);
		    ...
		    struct htlc_in_map *unconnected_htlcs_in STEALS)
{
	...
	for (hin = htlc_in_map_first(unconnected_htlcs_in, &ini); hin;
...
json_dev_ignore_htlcs(struct command *cmd, const char *buffer,
		      const jsmntok_t *obj UNNEEDED, const jsmntok_t *params)
{
	...
	if (!param_check(cmd, buffer, params,
			 ...
			 p_req("ignore", param_bool, &ignore), NULL))
...
static struct command_result *
param_channel(struct command *cmd, const char *name, const char *buffer,
	      const jsmntok_t *tok, struct channel **chan)
{
					     ...
					     const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params, p_opt("id", param_channel, &chan),
		   NULL))
		return command_param_failed();

	response = json_stream_success(cmd);
	json_array_start(response, "htlcs");
	for (i = wallet_htlcs_first(cmd, cmd->ld->wallet, chan, &scid, &htlc_id,
				    &cltv_expiry, &owner, &msat, &payment_hash,
				    &hstate);
	     i; i = wallet_htlcs_next(cmd->ld->wallet, i, &scid, &htlc_id,
				      &cltv_expiry, &owner, &msat,
				      &payment_hash, &hstate)) {
...
static const struct json_command listhtlcs_command = {
    "listhtlcs", "channels", json_listhtlcs,
