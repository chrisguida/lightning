...
impl From<responses::ListpeerchannelsChannels> for pb::ListpeerchannelsChannels {
    fn from(c: responses::ListpeerchannelsChannels) -> Self {
        Self {
            ...
            htlcs: c.htlcs.map(|arr| arr.into_iter().map(|i| i.into()).collect()).unwrap_or(vec![]), // Rule #3
            last_update_tx: c.last_update_tx, // Rule #2 for type string?
            last_settle_tx: c.last_settle_tx, // Rule #2 for type string?
            unbound_update_tx: c.unbound_update_tx, // Rule #2 for type string?
            unbound_settle_tx: c.unbound_settle_tx, // Rule #2 for type string?
            last_committed_settle_tx: c.last_committed_settle_tx, // Rule #2 for type string?
            close_to_addr: c.close_to_addr, // Rule #2 for type string?
        }
    }
}
