...
void db_bind_channel_type(struct db_stmt *stmt,
...
char *db_col_strdup(const tal_t *ctx, struct db_stmt *stmt,
		    const char *colname);
/* string or NULL */
...
struct amount_msat db_col_amount_msat(struct db_stmt *stmt,
				      const char *colname);
...
struct node_id *db_col_node_id_arr(const tal_t *ctx, struct db_stmt *stmt,
				   const char *colname);
void db_col_pubkey(struct db_stmt *stmt, const char *colname, struct pubkey *p);
...
#define db_col_arr(ctx, stmt, colname, type)                                   \
	((type *)db_col_arr_((ctx), (stmt), (colname), sizeof(type),           \
			     TAL_LABEL(type, "[]"), __func__))
...
#define db_col_optional(ctx, stmt, colname, type)                              \
	((struct type *)db_col_optional_(                                      \
	    tal(ctx, struct type), (stmt), (colname),                          \
	    typesafe_cb_cast(                                                  \
		void (*)(struct db_stmt *, const char *, void *),              \
		void (*)(struct db_stmt *, const char *, struct type *),       \
		db_col_##type)))

void *WARN_UNUSED_RESULT db_col_optional_(tal_t *dst, struct db_stmt *stmt,
					  const char *colname,
