...
void delete_channel(struct channel *channel STEALS)
{
	...
	if (hsm_capable(channel->peer->ld, WIRE_HSMD_FORGET_CHANNEL)) {
		msg = towire_hsmd_forget_channel(NULL, &channel->peer->id,
						 channel->dbid);
		msg = hsm_sync_req(tmpctx, channel->peer->ld, take(msg));
		if (!fromwire_hsmd_forget_channel_reply(msg))
			fatal("HSM gave bad hsm_forget_channel_reply %s",
			      tal_hex(msg, msg));
	}
...
bool maybe_cleanup_last_inflight(struct channel *channel)
{
	...
	wallet_channel_inflight_cleanup_incomplete(channel->peer->ld->wallet,
						   channel->dbid);
	tal_free(inflight);
	return true;
}
...
struct channel_inflight *new_inflight(
    ...
    u32 funding_feerate, struct amount_sat total_funds,
    struct amount_sat our_funds, struct wally_psbt *psbt STEALS,
    ...
    bool force_sign_first)
{
	...
	struct funding_info *funding = tal(inflight, struct funding_info);
	...
	inflight->lease_commit_sig = tal_dup_or_null(
	    inflight, secp256k1_ecdsa_signature, lease_commit_sig);
				    ...
				    u32 feerate_ppm)
{
	...
	channel->billboard.transient =
	    tal_fmt(channel, "%s", "Empty channel init'd");
	channel->log = new_logger(channel, ld->log_book, &peer->id,
				  "chan#%" PRIu64, channel->unsaved_dbid);
	...
	channel->type = default_channel_type(channel, ld->our_features,
					     peer->their_features);
...
struct amount_msat htlc_max_possible_send(const struct channel *channel)
{
	...
	if (!amount_sat_sub(
		&lower_bound, channel->funding_sats,
		channel->channel_info.their_config.channel_reserve)) {
		log_broken(
		    channel->log, "%s: their reserve %s > funding %s!",
		    __func__,
		    type_to_string(tmpctx, struct amount_sat,
				   &channel->funding_sats),
		    type_to_string(
			tmpctx, struct amount_sat,
			&channel->channel_info.their_config.channel_reserve));
	...
	if (!amount_sat_to_msat(&lower_bound_msat, lower_bound)) {
		log_broken(
		    channel->log, "%s: impossible size channel %s!", __func__,
		    type_to_string(tmpctx, struct amount_sat, &lower_bound));
		return AMOUNT_MSAT(0);
	}
...
struct channel *new_channel(
    ...
    /* NULL or stolen */
    struct wallet_shachain *their_shachain, enum channel_state state,
    enum side opener,
    /* NULL or stolen */
    struct logger *log, const char *transient_billboard TAKES, u8 channel_flags,
    bool req_confirmed_ins_local, bool req_confirmed_ins_remote,
    const struct channel_config *our_config, u32 minimum_depth,
    u64 next_index_local, u64 next_index_remote, u64 next_htlc_id,
    const struct bitcoin_outpoint *funding, struct amount_sat funding_sats,
    struct amount_msat push, struct amount_sat our_funds,
    bool remote_channel_ready,
    /* NULL or stolen */
    struct short_channel_id *scid, struct short_channel_id *alias_local STEALS,
    struct short_channel_id *alias_remote STEALS, struct channel_id *cid,
    struct amount_msat our_msat, struct amount_msat msat_to_us_min,
    struct amount_msat msat_to_us_max,
    /* NULL or stolen */
    struct bitcoin_tx *last_tx, const struct bitcoin_signature *last_sig,
    /* NULL or stolen */
    const struct bitcoin_signature *last_htlc_sigs,
    const struct channel_info *channel_info,
    const struct fee_states *fee_states TAKES,
    /* NULL or stolen */
    u8 *remote_shutdown_scriptpubkey, const u8 *local_shutdown_scriptpubkey,
    u64 final_key_idx, bool last_was_revoke,
    /* NULL or stolen */
    struct changed_htlc *last_sent_commit, u32 first_blocknum,
    ...
    u32 feerate_ppm, const u8 *remote_upfront_shutdown_script,
    ...
    struct nonce *our_next_nonce)
{
	...
	bool anysegwit =
	    ...
	    feature_negotiated(peer->ld->our_features, peer->their_features,
	...
	if (!log) {
		channel->log =
		    new_logger(channel, peer->ld->log_book, &channel->peer->id,
			       "chan#%" PRIu64, dbid);
	} else
	...
	channel->alias[REMOTE] =
	    tal_steal(channel, alias_remote); /* Haven't gotten one yet. */
	channel->cid = *cid;
	...
	if (last_sig) {
		...
		if (complete_update_tx) {
			channel->eltoo_keyset.complete_update_tx->chainparams =
			    chainparams;
		}
		if (complete_settle_tx) {
			channel->eltoo_keyset.complete_settle_tx->chainparams =
			    chainparams;
		}
		if (committed_update_tx) {
			channel->eltoo_keyset.committed_update_tx->chainparams =
			    chainparams;
		}
		if (committed_settle_tx) {
			channel->eltoo_keyset.committed_settle_tx->chainparams =
			    chainparams;
		}
	...
	if (local_shutdown_scriptpubkey) {
		...
		channel->shutdown_scriptpubkey[LOCAL] = p2tr_for_keyidx(
		    channel, channel->peer->ld, channel->final_key_idx);
	} else {
		channel->shutdown_scriptpubkey[LOCAL] = p2wpkh_for_keyidx(
		    channel, channel->peer->ld, channel->final_key_idx);
	}
	...
	if (!local_shutdown_scriptpubkey) {
		if (anysegwit) {
			txfilter_add_scriptpubkey(
			    peer->ld->owned_txfilter,
	...
	if (is_stub_scid(scid))
		channel->error = towire_errorfmt(
		    peer->ld, &channel->cid, "We can't be together anymore.");
...
peer_any_channel(struct peer *peer,
		 bool (*channel_state_filter)(enum channel_state), bool *others)
{
	struct channel *channel, *ret = NULL;

	list_for_each(&peer->channels, channel, list)
	{
		if (channel_state_filter &&
		    !channel_state_filter(channel->state))
			continue;
		/* Already found one? */
		if (ret) {
			if (others)
	...
	for (p = peer_node_id_map_first(ld->peers, &it); p;
		...
		list_for_each(&p->channels, chan, list)
		{
			...
			if (!privacy_leak_ok &&
				...
				continue;
			if (chan->scid && short_channel_id_eq(scid, chan->scid))
				return chan;
		}
	}
	return NULL;
				      ...
				      enum side side)
{
	...
	{
		if (c->alias[side] &&
		    short_channel_id_eq(c->alias[side], alias))
			return c;
	}
	return NULL;
}
		       ...
		       char *why)
{
	...
	if (channel_state_closing(state) && channel->closer == NUM_SIDES) {
		if (reason == REASON_LOCAL)
			channel->closer = LOCAL;
		if (reason == REASON_USER)
			channel->closer = LOCAL;
		if (reason == REASON_REMOTE)
			channel->closer = REMOTE;
		if (reason == REASON_ONCHAIN)
			channel->closer = REMOTE;
	}
	...
	if (state != old_state) { /* see issue #4029 */
		...
		wallet_state_change_add(channel->peer->ld->wallet,
					channel->dbid, timestamp, old_state,
					state, reason, why);
		notify_channel_state_changed(
		    channel->peer->ld, &channel->peer->id, &channel->cid,
		    channel->scid, timestamp, old_state, state, reason, why);
	}
}
			    ...
			    const char *fmt, ...)
{
	...
	if (!channel->error)
		channel->error =
		    towire_errorfmt(channel, &channel->cid, "%s", why);
...
struct channel_inflight *channel_current_inflight(const struct channel *channel)
{
	/* The last inflight should always be the one in progress */
	return list_tail(&channel->inflights, struct channel_inflight, list);
}
...
void channel_set_billboard(struct channel *channel, bool perm, const char *str)
{
	...
	*p = tal_free(*p);
	...
	if (str) {
		*p =
		    tal_fmt(channel, "%s:%s", channel_state_name(channel), str);
		if (taken(str))
			tal_free(str);
	}
}

static void channel_err(struct channel *channel, bool disconnect,
			const char *why)
{
