...
static void record_external_spend(const struct bitcoin_txid *txid,
				  struct tracked_output *out, u32 blockheight,
				  enum mvt_tag tag)
{
	send_coin_mvt(take(new_coin_external_spend(
	    NULL, &out->outpoint, txid, blockheight, out->sat, tag)));
}
...
{
	send_coin_mvt(take(new_coin_external_spend_tags(
	    NULL, &out->outpoint, txid, blockheight, out->sat, tags)));
}

static void record_external_output(const struct bitcoin_outpoint *out,
				   struct amount_sat amount, u32 blockheight,
				   enum mvt_tag tag)
{
	send_coin_mvt(take(
	    new_coin_external_deposit(NULL, out, blockheight, amount, tag)));
}
...
static void record_external_deposit_tags(const struct tracked_output *out,
					 ...
					 enum mvt_tag *tags TAKES)
{
	send_coin_mvt(take(new_coin_external_deposit_tags(
	    NULL, &out->outpoint, blockheight, out->sat, tags)));
}

static void record_mutual_close(const struct tx_parts *tx,
				const u8 *remote_scriptpubkey, u32 blockheight)
{
...
static void record_channel_deposit(struct tracked_output *out, u32 blockheight,
				   enum mvt_tag tag)
{
	send_coin_mvt(take(new_onchaind_deposit(NULL, &out->outpoint,
						blockheight, out->sat, tag)));
}
...
static void record_to_us_htlc_fulfilled(struct tracked_output *out,
					u32 blockheight)
{
	send_coin_mvt(take(new_onchain_htlc_deposit(
	    NULL, &out->outpoint, blockheight, out->sat, &out->payment_hash)));
}
					  ...
					  u32 blockheight)
{
	...
	send_coin_mvt(take(new_onchain_htlc_withdraw(
	    NULL, &out->outpoint, blockheight, out->sat, &out->payment_hash)));
}
...
static void record_anchor(struct tracked_output *out)
{
	...
	send_coin_mvt(take(new_coin_wallet_deposit_tagged(
	    NULL, &out->outpoint, out->tx_blockheight, out->sat, tags)));
}

static void record_coin_movements(struct tracked_output *out, u32 blockheight,
...
static bool grind_htlc_tx_fee(struct amount_sat *fee, struct bitcoin_tx *tx,
			      const struct bitcoin_signature *remotesig,
			      const u8 *wscript, u64 weight)
{
	...
	for (u64 i = min_possible_feerate; i <= max_possible_feerate; i++) {
		...
		*fee = amount_tx_fee(i, weight);
		...
		if (!check_tx_sig(tx, 0, NULL, wscript, &keyset->other_htlc_key,
				  remotesig))
			continue;
				 ...
				 const u8 *wscript)
{
	 ...
	 *    1. MUST be 0.
	 *  - Otherwise, MUST be calculated to match:
	 *    1. Multiply `feerate_per_kw` by 663 (666 if
	 * `option_anchor_outputs` applies) and divide by 1000 (rounding down).
	 */
	...
	if (amount_sat_eq(fee, AMOUNT_SAT(UINT64_MAX))) {
		...
		if (grind_htlc_tx_fee(&grindfee, tx, remotesig, wscript,
				      weight)) {
...
set_amount:
	...
	return check_tx_sig(tx, 0, NULL, wscript, &keyset->other_htlc_key,
			    remotesig);
}
...
static struct amount_sat get_htlc_success_fee(struct tracked_output *out)
{
	...
	weight += elements_tx_overhead(chainparams, 1, 1);
	if (!grind_htlc_tx_fee(&fee, tx, out->remote_htlc_sig, out->wscript,
			       weight)) {
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
			      "htlc_success_fee can't be found "
			      "for tx %s (weight %zu, feerate %u-%u), "
			      "signature %s, wscript %s",
			      type_to_string(tmpctx, struct bitcoin_tx, tx),
			      weight, min_possible_feerate,
			      max_possible_feerate,
	...
	if (!msg || !fromwire_hsmd_sign_tx_reply(msg, &sig)) {
		status_failed(STATUS_FAIL_HSM_IO, "Reading sign_tx_reply: %s",
			      tal_hex(tmpctx, msg));
	}

	witness =
	    bitcoin_witness_sig_and_element(tx, &sig, elem, elemsize, wscript);
 ...
 */
static struct bitcoin_tx *replace_penalty_tx_to_us(
    const tal_t *ctx,
    u8 *(*hsm_sign_msg)(const tal_t *ctx, struct bitcoin_tx *tx,
			const u8 *wscript),
    const struct bitcoin_tx *penalty_tx, struct amount_sat *output_amount)
{
	struct bitcoin_tx *tx;
	...
	input_wscript =
	    tal_dup_arr(tmpctx, u8, input->witness->items[2].witness,
			input->witness->items[2].witness_len, 0);
	input_element =
	    tal_dup_arr(tmpctx, u8, input->witness->items[1].witness,
			input->witness->items[1].witness_len, 0);
	input_amount = psbt_input_get_amount(penalty_tx->psbt, 0);

	/* Create the replacement.  */
	tx = bitcoin_tx(ctx, chainparams, 1, 1, /*locktime*/ 0);
	/* Reconstruct the input.  */
	bitcoin_tx_add_input(tx, &input_outpoint, BITCOIN_TX_RBF_SEQUENCE, NULL,
			     input_amount, NULL, input_wscript, NULL, NULL);
	/* Reconstruct the output with a smaller amount.  */
	if (amount_sat_greater(*output_amount, dust_limit)) {
		bitcoin_tx_add_output(
		    tx, scriptpubkey_p2wpkh(tx, &our_wallet_pubkey), NULL,
		    *output_amount);
		psbt_add_keypath_to_last_output(tx, our_wallet_index,
						&our_wallet_ext_key);
	} else {
		bitcoin_tx_add_output(tx, scriptpubkey_opreturn_padded(tx),
				      NULL, AMOUNT_SAT(0));
	...
	if (!wire_sync_write(HSM_FD,
			     take(hsm_sign_msg(NULL, tx, input_wscript))))
		status_failed(
		    STATUS_FAIL_HSM_IO,
		    "While feebumping penalty: writing sign request to hsm");
	msg = wire_sync_read(tmpctx, HSM_FD);
	if (!msg || !fromwire_hsmd_sign_tx_reply(msg, &sig))
		status_failed(
		    STATUS_FAIL_HSM_IO,
		    "While feebumping penalty: reading sign_tx_reply: %s",
		    tal_hex(tmpctx, msg));

	/* Install the witness with the signature.  */
	witness = bitcoin_witness_sig_and_element(
	    tx, &sig, input_element, tal_bytelen(input_element), input_wscript);
	bitcoin_tx_input_set_witness(tx, 0, take(witness));
 ...
 */
static struct amount_sat min_rbf_bump(size_t weight, size_t index)
{
	 ...
	 */
	if (mul_overflows_u64(
		index, min_relay_fee.satoshis)) /* Raw: multiplication.  */
		min_rbf_bump = AMOUNT_SAT(UINT64_MAX);
	else
		min_rbf_bump.satoshis =
		    index * min_relay_fee.satoshis; /* Raw: multiplication.  */
...
compute_penalty_output_amount(struct amount_sat initial_amount, u32 depth,
			      u32 max_depth, size_t weight)
{
	 ...
	 */
	if (!amount_sat_sub(&max_output_amount, initial_amount,
			    min_rbf_bump(weight, depth - 1)))
	...
	deducted_amount.satoshis =
	    (u64)(y * initial_amount.satoshis); /* Raw: multiplication.  */

	/* output_amount = initial_amount - deducted_amount.  */
	if (!amount_sat_sub(&output_amount, initial_amount, deducted_amount))
...
static void hsm_sign_local_htlc_tx(struct bitcoin_tx *tx, const u8 *wscript,
				   struct bitcoin_signature *sig)
{
	u8 *msg = towire_hsmd_sign_local_htlc_tx(NULL, commit_num, tx, wscript,
						 option_anchor_outputs);
...
static void hsm_get_per_commitment_point(struct pubkey *per_commitment_point)
{
	...
	if (!wire_sync_write(HSM_FD, take(msg)))
		status_failed(STATUS_FAIL_HSM_IO,
			      "Writing sign_htlc_tx to hsm");
	msg = wire_sync_read(tmpctx, HSM_FD);
	if (!msg || !fromwire_hsmd_get_per_commitment_point_reply(
			tmpctx, msg, per_commitment_point, &unused))
		status_failed(STATUS_FAIL_HSM_IO,
			      "Reading hsm_get_per_commitment_point_reply: %s",
			      tal_hex(tmpctx, msg));
}

static struct tracked_output *new_tracked_output(
    struct tracked_output ***outs, const struct bitcoin_outpoint *outpoint,
    u32 tx_blockheight, enum tx_type tx_type, struct amount_sat sat,
    enum output_type output_type, const struct htlc_stub *htlc,
    const u8 *wscript, const struct bitcoin_signature *remote_htlc_sig)
{
	...
	status_debug("Tracking output %s: %s/%s",
		     ...
		     tx_type_name(tx_type), output_type_name(output_type));
	...
	out->remote_htlc_sig =
	    tal_dup_or_null(out, struct bitcoin_signature, remote_htlc_sig);
...
{
	status_debug(
	    "Ignoring output %s: %s/%s",
	    type_to_string(tmpctx, struct bitcoin_outpoint, &out->outpoint),
...
static void handle_spend_created(struct tracked_output *out, const u8 *msg)
{
	...
	if (!fromwire_onchaind_spend_created(tmpctx, msg, &worthwhile,
					     &witness))
...
static struct proposed_resolution *
new_proposed_resolution(struct tracked_output *out, unsigned int block_required,
			enum tx_type tx_type)
{
	struct proposed_resolution *proposal =
	    tal(out, struct proposed_resolution);
					 ...
					 enum tx_type tx_type)
{
	...
	status_debug("Telling lightningd about %s to resolve %s/%s"
		     ...
		     tx_type_name(tx_type), tx_type_name(out->tx_type),
		     output_type_name(out->output_type), block_required - 1,
		     block_required - 1 - out->tx_blockheight);
	...
	handle_spend_created(
	    out, queue_until_msg(tmpctx, WIRE_ONCHAIND_SPEND_CREATED));
}
...
static void propose_ignore(struct tracked_output *out,
			   unsigned int block_required, enum tx_type tx_type)
{
	status_debug("Propose ignoring %s/%s as %s"
		     " after block %u (%i more blocks)",
		     tx_type_name(out->tx_type),
		     output_type_name(out->output_type), tx_type_name(tx_type),
		     block_required, block_required - out->tx_blockheight);
...
static bool onchain_witness_element_matches(
    const struct onchain_witness_element **welements,
    const struct wally_tx_input *input)
{
	...
	for (size_t i = 0; i < stack->num_items; i++) {
		...
		if (welements[i]->is_signature)
			continue;
		if (!memeq(stack->items[i].witness, stack->items[i].witness_len,
			   welements[i]->witness,
				 ...
				 const struct tx_parts *tx_parts)
{
	...
	status_debug(
	    "Resolved %s/%s by our proposal %s (%s)",
	    tx_type_name(out->tx_type), output_type_name(out->output_type),
	    tx_type_name(out->proposal->tx_type),
	    type_to_string(tmpctx, struct bitcoin_txid, &out->resolved->txid));
			      ...
			      enum tx_type tx_type)
{
	...
	status_debug("Resolved %s/%s by %s (%s)", tx_type_name(out->tx_type),
		     output_type_name(out->output_type), tx_type_name(tx_type),
			  ...
			  const struct tx_parts *tx_parts)
{
	...
	status_broken(
	    "Unknown spend of %s/%s by %s", tx_type_name(out->tx_type),
	    output_type_name(out->output_type),
	    type_to_string(tmpctx, struct bitcoin_txid, &tx_parts->txid));
}

static u64 unmask_commit_number(const struct tx_parts *tx, uint32_t locktime,
				enum side opener,
				...
				const struct pubkey *remote_payment_basepoint)
{
	 ...
	 *...
	 * * `txin[0]` sequence: upper 8 bits are 0x80, lower 24 bits are upper
...
static void billboard_update(struct tracked_output **outs)
{
	...
	if (best) {
		...
		if (best->proposal->depth_required <= best->depth) {
			peer_billboard(
			    false,
			    "%u outputs unresolved: waiting confirmation that "
			    "we spent %s (%s) using %s",
			...
			peer_billboard(
			    false,
			    "%u outputs unresolved: in %u blocks will spend %s "
			    "(%s) using %s",
			    num_not_irrevocably_resolved(outs),
...
static void
handle_htlc_onchain_fulfill(struct tracked_output *out,
			    const struct tx_parts *tx_parts,
			    const struct bitcoin_outpoint *htlc_outpoint)
{
	...
	if (out->tx_type == THEIR_UNILATERAL ||
		 ...
		 * <localhtlcsig> <payment_preimage>` for HTLC-success
		 */
		if (tx_parts->inputs[htlc_outpoint->n]->witness->num_items !=
		    5) /* +1 for wscript */
			status_failed(STATUS_FAIL_INTERNAL_ERROR,
				      "%s/%s spent with weird witness %zu",
				      tx_type_name(out->tx_type),
				      output_type_name(out->output_type),
				      tx_parts->inputs[htlc_outpoint->n]
					  ->witness->num_items);

		preimage_item =
		    &tx_parts->inputs[htlc_outpoint->n]->witness->items[3];
		...
		preimage_item =
		    &tx_parts->inputs[htlc_outpoint->n]->witness->items[1];
	} else
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
	...
	status_debug("%s/%s gave us preimage %s", tx_type_name(out->tx_type),
		     output_type_name(out->output_type),
		     type_to_string(tmpctx, struct preimage, &preimage));
	wire_sync_write(
	    REQ_FD, take(towire_onchaind_extracted_preimage(NULL, &preimage)));
}

static void resolve_htlc_tx(struct tracked_output ***outs, size_t out_index,
			    const struct tx_parts *htlc_tx, size_t input_num,
			    u32 tx_blockheight)
{
	struct tracked_output *out;
	...
	struct bitcoin_outpoint outpoint;
	u8 *msg;
	u8 *wscript = bitcoin_wscript_htlc_tx(
	    tmpctx, to_self_delay[LOCAL], &keyset->self_revocation_key,
	...
	out = new_tracked_output(outs, &outpoint, tx_blockheight,
				 (*outs)[out_index]->resolved->tx_type, amt,
				 DELAYED_OUTPUT_TO_US, NULL, NULL, NULL);

	msg = towire_onchaind_spend_to_us(
	    NULL, &outpoint, amt, rel_blockheight(out, to_self_delay[LOCAL]),
	    commit_num, wscript);
...
/* BOLT #5:
 *
 *   - MUST *resolve* the _remote node's HTLC-timeout transaction_ by spending
 * it using the revocation private key.
 *   - MUST *resolve* the _remote node's HTLC-success transaction_ by spending
 * it using the revocation private key.
 */
static void steal_htlc_tx(struct tracked_output *out,
			  struct tracked_output ***outs,
			  const struct tx_parts *htlc_tx,
			  u32 htlc_tx_blockheight, enum tx_type htlc_tx_type,
			  const struct bitcoin_outpoint *htlc_outpoint)
{
	...
	u8 *wscript = bitcoin_wscript_htlc_tx(
	    htlc_tx, to_self_delay[REMOTE], &keyset->self_revocation_key,
	    &keyset->self_delayed_payment_key);
	...
	htlc_out = new_tracked_output(outs, htlc_outpoint, htlc_tx_blockheight,
				      htlc_tx_type, htlc_out_amt,
				      DELAYED_CHEAT_OUTPUT_TO_THEM, &out->htlc,
				      wscript, NULL);
	 ...
	 * `<revocationsig> 1`
	 */
	msg = towire_onchaind_spend_penalty(
	    NULL, htlc_outpoint, htlc_out_amt, remote_per_commitment_secret,
...
static void output_spent(struct tracked_output ***outs,
			 const struct tx_parts *tx_parts, u32 input_num,
			 u32 tx_blockheight)
{
	for (size_t i = 0; i < tal_count(*outs); i++) {
		...
		case THEIR_HTLC:
			if (out->tx_type == THEIR_REVOKED_UNILATERAL) {
				...
				record_external_deposit_tags(
				    out, out->tx_blockheight,
				    /* This takes tags */
				    tal_dup_talarr(NULL, enum mvt_tag, tags));
				record_external_spend_tags(
				    &tx_parts->txid, out, tx_blockheight, tags);

				/* we've actually got a 'new' output here */
				steal_htlc_tx(
				    out, outs, tx_parts, tx_blockheight,
				    THEIR_HTLC_TIMEOUT_TO_THEM, &htlc_outpoint);
			} else {
				/* We ignore this timeout tx, since we should
				 * resolve by ignoring once we reach depth. */
				onchain_annotate_txout(&htlc_outpoint,
						       TX_CHANNEL_HTLC_TIMEOUT |
							   TX_THEIRS);
			}
			break;
		...
		case OUR_HTLC:
			...
			/* BOLT #5:
			 *
			 * ## HTLC Output Handling: Local Commitment, Local
			 *Offers
			 *...
			 *    - MUST extract the payment preimage from the
			 *      transaction input witness.
			 *...
			 * ## HTLC Output Handling: Remote Commitment, Local
			 *Offers
			 *...
			 *     - MUST extract the payment preimage from the
			...
			if (out->tx_type == THEIR_REVOKED_UNILATERAL) {
				enum mvt_tag *tags =
				    new_tag_arr(NULL, HTLC_FULFILL);
				tal_arr_expand(&tags, STEALABLE);
				record_external_spend_tags(
				    &tx_parts->txid, out, tx_blockheight, tags);
				steal_htlc_tx(
				    out, outs, tx_parts, tx_blockheight,
				    OUR_HTLC_FULFILL_TO_THEM, &htlc_outpoint);
			} else {
				 ...
				 *    output is considered *irrevocably
				 *resolved*
				 */
				ignore_output(out);

				onchain_annotate_txout(&htlc_outpoint,
						       TX_CHANNEL_HTLC_SUCCESS |
							   TX_THEIRS);
			}
			break;
...
static void update_resolution_depth(struct tracked_output *out, u32 depth)
{
	...
	status_debug("%s/%s->%s depth %u", tx_type_name(out->tx_type),
		     output_type_name(out->output_type),
		     tx_type_name(out->resolved->tx_type), depth);
			 ...
			 const struct bitcoin_txid *txid, u32 depth)
{
	...
	if (bitcoin_txid_eq(&outs[0]->resolved->txid, txid) &&
	    depth >= reasonable_depth && missing_htlc_msgs) {
	...
	for (i = 0; i < tal_count(outs); i++) {
		...
		if (outs[i]->proposal &&
			...
			if (outs[i]->proposal->tx_type ==
			    THEIR_HTLC_TIMEOUT_TO_THEM)
				record_external_deposit(outs[i],
							outs[i]->tx_blockheight,
							HTLC_TIMEOUT);
		}
	}
}
 ...
 *
 * A local node:
 *   - if it receives (or already possesses) a payment preimage for an
 *unresolved HTLC output that it has been offered AND for which it has committed
 *to an outgoing HTLC:
			    ...
			    const struct preimage *preimage)
{
	...
	for (i = 0; i < tal_count(outs); i++) {
		...
		if (outs[i]->remote_htlc_sig) {
			...
			htlc_wscript = bitcoin_wscript_htlc_tx(
			    tmpctx, to_self_delay[LOCAL],
			    &keyset->self_revocation_key,
			    &keyset->self_delayed_payment_key);

			fee = get_htlc_success_fee(outs[i]);
			msg = towire_onchaind_spend_htlc_success(
			    NULL, &outs[i]->outpoint, outs[i]->sat, fee,
			    outs[i]->htlc.id, commit_num,
			    outs[i]->remote_htlc_sig, preimage,
			    outs[i]->wscript, htlc_wscript);
			 ...
			 *      convenient address.
			 */
			msg = towire_onchaind_spend_fulfill(
			    NULL, &outs[i]->outpoint, outs[i]->sat,
			    outs[i]->htlc.id, remote_per_commitment_point,
			    preimage, outs[i]->wscript);
...
static void handle_dev_memleak(struct tracked_output ***outs, const u8 *msg)
{
	...
	wire_sync_write(
	    REQ_FD, take(towire_onchaind_dev_memleak_reply(NULL, found_leak)));
}
 ...
 *      - MUST monitor the blockchain for transactions that spend any output
 * that is NOT *irrevocably resolved*.
 */
...
static int cmp_htlc_with_tells_cltv(const struct htlc_with_tells *a,
				    const struct htlc_with_tells *b,
				    void *unused)
{
...
static struct htlcs_info *init_reply(const tal_t *ctx, const char *what)
{
	...
	if (!fromwire_onchaind_htlcs(htlcs_info, msg, &htlcs_info->htlcs,
	...
	htlcs = tal_arr(tmpctx, struct htlc_with_tells,
			tal_count(htlcs_info->htlcs));
	...
	htlcs_info->htlcs =
	    tal_arr(htlcs_info, struct htlc_stub, tal_count(htlcs));
	htlcs_info->tell_if_missing =
	    tal_arr(htlcs_info, bool, tal_count(htlcs));
	htlcs_info->tell_immediately =
	    tal_arr(htlcs_info, bool, tal_count(htlcs));
	for (size_t i = 0; i < tal_count(htlcs); i++) {
...
static u8 **derive_htlc_scripts(const struct htlc_stub *htlcs, enum side side)
{
	...
	for (i = 0; i < tal_count(htlcs); i++) {
		if (htlcs[i].owner == side)
			htlc_scripts[i] = htlc_offered_wscript(
			    htlc_scripts, &htlcs[i].ripemd, keyset,
			    option_anchor_outputs,
			    option_anchors_zero_fee_htlc_tx);
		else {
			...
			if (!blocks_to_abs_locktime(htlcs[i].cltv_expiry,
						    &ltime))
				status_failed(STATUS_FAIL_INTERNAL_ERROR,
					      "Could not convert cltv_expiry "
					      "%u to locktime",
					      htlcs[i].cltv_expiry);
			htlc_scripts[i] = htlc_received_wscript(
			    htlc_scripts, &htlcs[i].ripemd, &ltime, keyset,
			    option_anchor_outputs,
			    option_anchors_zero_fee_htlc_tx);
		}
	}
					 ...
					 u8 **htlc_scripts)
{
	...
	for (i = 0; i < tal_count(matches); i++) {
		...
		if (i > 0 && (htlcs[matches[i]].cltv_expiry ==
		 ...
		 */
		tx = htlc_timeout_tx(tmpctx, chainparams, &out->outpoint,
	...
	if (i == tal_count(matches)) {
		...
		status_failed(
		    ...
		    " cltvs %s wscripts %s"
		    "%s%s",
		    tal_count(matches), min_possible_feerate,
		    max_possible_feerate,
		    type_to_string(tmpctx, struct bitcoin_tx, tx),
		    type_to_string(tmpctx, struct amount_sat, &out->sat),
		    type_to_string(tmpctx, struct bitcoin_signature,
	...
	msg = towire_onchaind_spend_htlc_timeout(
	    NULL, &out->outpoint, out->sat, fee, htlcs[matches[i]].id,
	    htlcs[matches[i]].cltv_expiry, commit_num, out->remote_htlc_sig,
	    htlc_scripts[matches[i]], htlc_wscript);

	propose_resolution_to_master(
	    out, take(msg),
	    /* nLocktime: we have to be *after* that block! */
	    htlcs[matches[i]].cltv_expiry + 1, OUR_HTLC_TIMEOUT_TX);
	return matches[i];
}
...
static u32 matches_cltv(const size_t *matches, const struct htlc_stub *htlcs)
{
					   ...
					   u8 **htlc_scripts)
{
	 ...
	 *       address.
	 */
	msg = towire_onchaind_spend_htlc_expired(
	    NULL, &out->outpoint, out->sat, htlc->id, cltv_expiry,
	    remote_per_commitment_point, htlc_scripts[matches[0]]);
	propose_resolution_to_master(
	    out, take(msg),
	    /* nLocktime: we have to be *after* that block! */
	    cltv_expiry + 1, OUR_HTLC_TIMEOUT_TO_US);
				 ...
				 u8 **htlc_scripts)
{
	...
	for (size_t i = 1; i < tal_count(matches); i++) {
		if (htlcs[matches[i]].cltv_expiry >
		    htlcs[which_htlc].cltv_expiry)
				       ...
				       u8 **htlc_scripts)
{
	...
	const u8 *script =
	    tal_dup_arr(tmpctx, u8, out->script, out->script_len, 0);
			       ...
			       const struct htlcs_info *htlcs_info)
{
	for (size_t i = 0; i < tal_count(htlcs_info->htlcs); i++) {
		...
		msg = towire_onchaind_missing_htlc_output(
		    missing_htlc_msgs, &htlcs_info->htlcs[i]);
...
static void get_anchor_scriptpubkeys(const tal_t *ctx, u8 **anchor)
{
	...
	for (enum side side = 0; side < NUM_SIDES; side++) {
		u8 *wscript =
		    bitcoin_wscript_anchor(tmpctx, &funding_pubkey[side]);
		anchor[side] = scriptpubkey_p2wsh(ctx, wscript);
	}
}

static u8 *scriptpubkey_to_remote(const tal_t *ctx,
				  const struct pubkey *remotekey, u32 csv_lock)
{
	/* BOLT #3:
	 *
	 * #### `to_remote` Output
	 ...
	 */
	if (option_anchor_outputs || option_anchors_zero_fee_htlc_tx) {
		return scriptpubkey_p2wsh(
		    ctx, bitcoin_wscript_to_remote_anchored(tmpctx, remotekey,
							    csv_lock));
	} else {
...
static void our_unilateral_to_us(struct tracked_output ***outs,
				 const struct bitcoin_outpoint *outpoint,
				 u32 tx_blockheight, struct amount_sat amt,
				 u16 sequence, const u8 *local_scriptpubkey,
				 const u8 *local_wscript)
{
	 ...
	 *       the output.
	 */
	out = new_tracked_output(outs, outpoint, tx_blockheight, OUR_UNILATERAL,
				 amt, DELAYED_OUTPUT_TO_US, NULL, NULL, NULL);

	msg = towire_onchaind_spend_to_us(
	    NULL, outpoint, amt, rel_blockheight(out, to_self_delay[LOCAL]),
	    commit_num, local_wscript);
...
static void
handle_our_unilateral(const struct tx_parts *tx, u32 tx_blockheight,
		      ...
		      struct tracked_output **outs)
{
	...
	if (!derive_keyset(&local_per_commitment_point, &basepoints[LOCAL],
			   &basepoints[REMOTE],
			   commit_num >= static_remotekey_start[LOCAL], ks))
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
			      "Deriving keyset for %" PRIu64, commit_num);

	status_debug(
	    "Deconstructing unilateral tx: %" PRIu64 " using keyset: "
	    ...
	    " other_htlc_key: %s",
	    commit_num,
	    type_to_string(tmpctx, struct pubkey, &keyset->self_revocation_key),
	    type_to_string(tmpctx, struct pubkey,
			   &keyset->self_delayed_payment_key),
	    type_to_string(tmpctx, struct pubkey, &keyset->self_payment_key),
	    type_to_string(tmpctx, struct pubkey, &keyset->other_payment_key),
	    type_to_string(tmpctx, struct pubkey, &keyset->self_htlc_key),
	    type_to_string(tmpctx, struct pubkey, &keyset->other_htlc_key));

	local_wscript =
	    to_self_wscript(tmpctx, to_self_delay[LOCAL], 1, keyset);
	...
	script[REMOTE] =
	    scriptpubkey_to_remote(tmpctx, &keyset->other_payment_key, 1);

	/* Calculate all the HTLC scripts so we can match them */
	htlc_scripts = derive_htlc_scripts(htlcs_info->htlcs, LOCAL);

	status_debug("Script to-me: %u: %s (%s)", to_self_delay[LOCAL],
		     tal_hex(tmpctx, script[LOCAL]),
		     tal_hex(tmpctx, local_wscript));
	status_debug("Script to-them: %s", tal_hex(tmpctx, script[REMOTE]));
	...
	for (i = 0; i < tal_count(tx->outputs); i++) {
		...
		size_t which_htlc;
		struct amount_asset asset =
		    wally_tx_output_get_amount(tx->outputs[i]);
		struct amount_sat amt;
		struct bitcoin_outpoint outpoint;
		...
		if (chainparams->is_elements &&
			 ...
			 * fee output. */
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, OUR_UNILATERAL,
			    amt, ELEMENTS_FEE, NULL, NULL, NULL);
			ignore_output(out);
			continue;
			...
			our_unilateral_to_us(&outs, &outpoint, tx_blockheight,
					     ...
					     script[LOCAL], local_wscript);

			script[LOCAL] = NULL;
			continue;
		}
		if (script[REMOTE] &&
		    wally_tx_output_scripteq(tx->outputs[i], script[REMOTE])) {
			/* BOLT #5:
			 *
			...
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, OUR_UNILATERAL,
			    amt, OUTPUT_TO_THEM, NULL, NULL, NULL);
			ignore_output(out);
		...
		if (anchor[LOCAL] &&
		    wally_tx_output_scripteq(tx->outputs[i], anchor[LOCAL])) {
			/* FIXME: We should be able to spend this! */
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, OUR_UNILATERAL,
			    amt, ANCHOR_TO_US, NULL, NULL, NULL);
			ignore_output(out);
			record_anchor(out);
			anchor[LOCAL] = NULL;
			continue;
		...
		if (anchor[REMOTE] &&
		    wally_tx_output_scripteq(tx->outputs[i], anchor[REMOTE])) {
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, OUR_UNILATERAL,
			    amt, ANCHOR_TO_THEM, NULL, NULL, NULL);
			ignore_output(out);
		...
		matches =
		    match_htlc_output(tmpctx, tx->outputs[i], htlc_scripts);
		...
		if (tal_count(matches) == 0) {
			...
			if (opener == REMOTE && script[LOCAL]) {
				...
				/* We already tried `1` */
				for (size_t csv = 2; csv <= LEASE_RATE_DURATION;
				     csv++) {

					local_wscript = to_self_wscript(
					    tmpctx, to_self_delay[LOCAL], csv,
					    keyset);

					script[LOCAL] = scriptpubkey_p2wsh(
					    tmpctx, local_wscript);
					if (!wally_tx_output_scripteq(
						tx->outputs[i], script[LOCAL]))
						continue;

					our_unilateral_to_us(
					    &outs, &outpoint, tx_blockheight,
					    amt,
					    max_unsigned(to_self_delay[LOCAL],
							 csv),
					    script[LOCAL], local_wscript);
				...
				for (size_t csv = 2; csv <= LEASE_RATE_DURATION;
					...
					script[REMOTE] = scriptpubkey_to_remote(
					    tmpctx, &keyset->other_payment_key,
					    csv);

					if (!wally_tx_output_scripteq(
						tx->outputs[i], script[REMOTE]))
						continue;

					/* BOLT #5:
					 *
					 *     - MAY ignore the `to_remote`
					 * output.
					 *       - Note: No action is required
					 * by the local node, as `to_remote` is
					 * considered *resolved* by the
					 * commitment transaction itself.
					 */
					out = new_tracked_output(
					    &outs, &outpoint, tx_blockheight,
					    OUR_UNILATERAL, amt, OUTPUT_TO_THEM,
					    NULL, NULL, NULL);
					ignore_output(out);
					record_external_deposit(
					    out, tx_blockheight, TO_THEM);
					script[REMOTE] = NULL;
					found = true;
					break;
				}
			...
			onchain_annotate_txout(&outpoint,
					       TX_CHANNEL_PENALTY | TX_THEIRS);

			record_external_output(&outpoint, amt, tx_blockheight,
					       PENALTY);
			status_failed(
			    STATUS_FAIL_INTERNAL_ERROR,
			    "Could not find resolution for output %zu", i);
		}

		if (matches_direction(matches, htlcs_info->htlcs) == LOCAL) {
			/* BOLT #5:
			 *
			 *     - MUST handle HTLCs offered by itself as
			 * specified in [HTLC Output Handling: Local Commitment,
			 *       Local Offers]
			 */
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, OUR_UNILATERAL,
			    amt, OUR_HTLC, NULL, NULL, remote_htlc_sigs);
			/* Tells us which htlc to use */
			which_htlc = resolve_our_htlc_ourcommit(
			    out, matches, htlcs_info->htlcs, htlc_scripts);
		} else {
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, OUR_UNILATERAL,
			    amt, THEIR_HTLC, NULL, NULL, remote_htlc_sigs);
			/* BOLT #5:
			 *
...
static void steal_to_them_output(struct tracked_output *out, u32 csv)
{
	...
	msg = towire_onchaind_spend_penalty(NULL, &out->outpoint, out->sat,
					    remote_per_commitment_secret,
					    tal_dup(tmpctx, u8, &ONE), wscript);
...
static void steal_htlc(struct tracked_output *out)
{
	...
	msg = towire_onchaind_spend_penalty(
	    NULL, &out->outpoint, out->sat, remote_per_commitment_secret,
	    tal_dup_arr(tmpctx, u8, der, ARRAY_SIZE(der), 0), out->wscript);
...
static void tell_wallet_to_remote(const struct tx_parts *tx,
				  const struct bitcoin_outpoint *outpoint,
				  u32 tx_blockheight, const u8 *scriptpubkey,
				  const struct pubkey *per_commit_point,
				  bool option_static_remotekey, u32 csv_lock)
{
	struct amount_asset asset =
	    wally_tx_output_get_amount(tx->outputs[outpoint->n]);
	struct amount_sat amt;
	...
	wire_sync_write(REQ_FD, take(towire_onchaind_add_utxo(
				    NULL, outpoint, per_commit_point, amt,
				    tx_blockheight, scriptpubkey, csv_lock)));
}
...
static void their_unilateral_local(struct tracked_output ***outs,
				   ...
				   u32 tx_blockheight, struct amount_sat amt,
				   const u8 *local_scriptpubkey,
				   enum tx_type tx_type, u32 csv_lock)
{
	struct tracked_output *out;
	/* BOLT #5:
	 *
	 ...
	 *     commitment transaction itself.
	 */
	out = new_tracked_output(outs, outpoint, tx_blockheight, tx_type, amt,
				 OUTPUT_TO_US, NULL, NULL, NULL);
	ignore_output(out);

	tell_wallet_to_remote(tx, outpoint, tx_blockheight, local_scriptpubkey,
 ...
 */
static void handle_their_cheat(const struct tx_parts *tx, u32 tx_blockheight,
			       ...
			       struct tracked_output **outs)
{
	...
	htlcs_info =
	    init_reply(tx, "Tracking their illegal close: taking all funds");
	...
	remote_per_commitment_secret =
	    tal_dup(tx, struct secret, (struct secret *)revocation_preimage);
	...
	status_debug(
	    "Deriving keyset %" PRIu64 ": per_commit_point=%s"
	    ...
	    commit_num,
	    type_to_string(tmpctx, struct pubkey, remote_per_commitment_point),
	    type_to_string(tmpctx, struct pubkey, &basepoints[REMOTE].payment),
	    type_to_string(tmpctx, struct pubkey, &basepoints[LOCAL].payment),
	    type_to_string(tmpctx, struct pubkey, &basepoints[REMOTE].htlc),
	    type_to_string(tmpctx, struct pubkey, &basepoints[LOCAL].htlc),
	    type_to_string(tmpctx, struct pubkey,
	...
	if (!derive_keyset(remote_per_commitment_point, &basepoints[REMOTE],
			   &basepoints[LOCAL],
			   commit_num >= static_remotekey_start[REMOTE], ks))
		status_failed(STATUS_FAIL_INTERNAL_ERROR,
			      "Deriving keyset for %" PRIu64, commit_num);

	status_debug(
	    "Deconstructing revoked unilateral tx: %" PRIu64 " using keyset: "
	    " self_revocation_key: %s"
	    " self_delayed_payment_key: %s"
	    " self_payment_key: %s"
	    " other_payment_key: %s"
	    ...
	    type_to_string(tmpctx, struct pubkey, &keyset->self_htlc_key),
	    type_to_string(tmpctx, struct pubkey, &keyset->other_htlc_key),
	    static_remotekey_start[LOCAL], static_remotekey_start[REMOTE]);

	remote_wscript =
	    to_self_wscript(tmpctx, to_self_delay[REMOTE], 1, keyset);
	...
	status_debug("Script to-them: %u: %s (%s)", to_self_delay[REMOTE],
	...
	for (i = 0; i < tal_count(tx->outputs); i++) {
		...
		if (chainparams->is_elements &&
			 ...
			 * fee output. */
			out =
			    new_tracked_output(&outs, &outpoint, tx_blockheight,
					       THEIR_REVOKED_UNILATERAL, amt,
					       ELEMENTS_FEE, NULL, NULL, NULL);
			ignore_output(out);
			continue;
		}

		if (script[LOCAL] &&
		    wally_tx_output_scripteq(tx->outputs[i], script[LOCAL])) {
			their_unilateral_local(
			    &outs, tx, &outpoint, tx_blockheight, amt,
			    script[LOCAL], THEIR_REVOKED_UNILATERAL, 1);
			script[LOCAL] = NULL;
			continue;
		}
		if (script[REMOTE] &&
			...
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight,
			    THEIR_REVOKED_UNILATERAL, amt,
			    DELAYED_CHEAT_OUTPUT_TO_THEM, NULL, NULL, NULL);
		...
		if (anchor[REMOTE] &&
			...
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight,
			    THEIR_REVOKED_UNILATERAL, amt, ANCHOR_TO_THEM, NULL,
			    NULL, NULL);
			ignore_output(out);
			record_external_deposit(out, tx_blockheight, ANCHOR);
			anchor[REMOTE] = NULL;
		...
		if (tal_count(matches) == 0) {
			...
			if (opener == REMOTE && script[LOCAL]) {
				...
				for (size_t csv = 2; csv <= LEASE_RATE_DURATION;
				     csv++) {
					script[LOCAL] = scriptpubkey_to_remote(
					    tmpctx, &keyset->other_payment_key,
					    csv);
					if (!wally_tx_output_scripteq(
						tx->outputs[i], script[LOCAL]))
					...
					their_unilateral_local(
					    &outs, tx, &outpoint,
					    tx_blockheight, amt, script[LOCAL],
					    THEIR_REVOKED_UNILATERAL, csv);
					script[LOCAL] = NULL;
					found = true;
					break;
				}
				...
				for (size_t csv = 2; csv <= LEASE_RATE_DURATION;
				     csv++) {
					remote_wscript = to_self_wscript(
					    tmpctx, to_self_delay[REMOTE], csv,
					    keyset);
					script[REMOTE] = scriptpubkey_p2wsh(
					    tmpctx, remote_wscript);
					...
					out = new_tracked_output(
					    &outs, &outpoint, tx_blockheight,
					    THEIR_REVOKED_UNILATERAL, amt,
					    DELAYED_CHEAT_OUTPUT_TO_THEM, NULL,
					    NULL, NULL);
		...
		if (matches_direction(matches, htlcs_info->htlcs) == LOCAL) {
			 ...
			 * has passed.
			 *     * spend the *HTLC-success tx*, if the remote node
			 * has published it.
			 */
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight,
			    THEIR_REVOKED_UNILATERAL, amt, OUR_HTLC,
			...
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight,
			    THEIR_REVOKED_UNILATERAL, amt, THEIR_HTLC,
			    &htlcs_info->htlcs[which_htlc],
			    htlc_scripts[which_htlc], NULL);
			/* BOLT #5:
			 *
...
static void
handle_their_unilateral(const struct tx_parts *tx, u32 tx_blockheight,
			...
			const enum side opener, struct tracked_output **outs)
{
	...
	for (i = 0; i < tal_count(tx->outputs); i++) {
		...
		if (chainparams->is_elements &&
			 ...
			 * fee output. */
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, THEIR_UNILATERAL,
			    amt, ELEMENTS_FEE, NULL, NULL, NULL);
			ignore_output(out);
			continue;
		} else if (script[LOCAL] &&
			...
			their_unilateral_local(
			    &outs, tx, &outpoint, tx_blockheight, amt,
			    script[LOCAL], THEIR_UNILATERAL, 1);
		...
		if (script[REMOTE] &&
			...
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, THEIR_UNILATERAL,
			    amt, DELAYED_OUTPUT_TO_THEM, NULL, NULL, NULL);
			ignore_output(out);
			record_external_deposit(out, tx_blockheight, TO_THEM);
			continue;
		}
		...
		if (tal_count(matches) == 0) {
			...
			if (opener == REMOTE && script[LOCAL]) {
				...
				for (size_t csv = 2; csv <= LEASE_RATE_DURATION;
					...
					out = new_tracked_output(
					    &outs, &outpoint, tx_blockheight,
					    THEIR_UNILATERAL, amt,
					    DELAYED_OUTPUT_TO_THEM, NULL, NULL,
					    NULL);
					ignore_output(out);
					record_external_deposit(
					    out, tx_blockheight, TO_THEM);
		...
		if (matches_direction(matches, htlcs_info->htlcs) == LOCAL) {
			...
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, THEIR_UNILATERAL,
			    amt, OUR_HTLC, NULL, NULL, NULL);
			which_htlc = resolve_our_htlc_theircommit(
			    out, matches, htlcs_info->htlcs, htlc_scripts);
		} else {
			out = new_tracked_output(
			    &outs, &outpoint, tx_blockheight, THEIR_UNILATERAL,
			    amt, THEIR_HTLC, NULL, NULL, NULL);
			/* BOLT #5:
			 *
...
static void handle_unknown_commitment(
    const struct tx_parts *tx, u32 tx_blockheight,
    const struct pubkey *possible_remote_per_commitment_point,
    const struct basepoints basepoints[NUM_SIDES], struct tracked_output **outs)
{
	int to_us_output = -1;
	...
	if (possible_remote_per_commitment_point) {
		...
		if (!derive_keyset(possible_remote_per_commitment_point,
				   &basepoints[REMOTE], &basepoints[LOCAL],
				   false, ks))
			status_failed(
			    STATUS_FAIL_INTERNAL_ERROR,
			    "Deriving keyset for "
			    "possible_remote_per_commitment_point %s",
			    type_to_string(
				tmpctx, struct pubkey,
				possible_remote_per_commitment_point));

		local_scripts[0] =
		    scriptpubkey_p2wpkh(tmpctx, &ks->other_payment_key);
	} else {
	...
	for (size_t csv = 1; csv <= LEASE_RATE_DURATION; csv++) {
		...
		local_scripts[1] = scriptpubkey_to_remote(
		    tmpctx, &basepoints[LOCAL].payment, csv);
		...
		for (size_t i = 0; i < tal_count(tx->outputs); i++) {
			...
			out =
			    new_tracked_output(&outs, &outpoint, tx_blockheight,
					       UNKNOWN_UNILATERAL, amt,
					       OUTPUT_TO_US, NULL, NULL, NULL);
			ignore_output(out);

			tell_wallet_to_remote(
			    tx, &outpoint, tx_blockheight,
	...
	if (to_us_output == -1) {
		status_broken("FUNDS LOST.  Unknown commitment #%" PRIu64 "!",
			      commit_num);
		htlcs_info =
		    init_reply(tx, "ERROR: FUNDS LOST.  Unknown commitment!");
	} else {
		status_broken("ERROR: Unknown commitment #%" PRIu64
			      ", recovering our funds!",
			      commit_num);
		htlcs_info = init_reply(
		    tx, "ERROR: Unknown commitment, recovering our funds!");
	}
...
int main(int argc, char *argv[])
{
	...
	if (!fromwire_onchaind_init(
		tmpctx, msg, &shachain, &chainparams, &funding_sats, &our_msat,
		&old_remote_per_commit_point, &remote_per_commit_point,
		&to_self_delay[LOCAL], &to_self_delay[REMOTE], &dust_limit,
		&our_broadcast_txid, &scriptpubkey[LOCAL],
		&scriptpubkey[REMOTE], &our_wallet_index, &our_wallet_ext_key,
		&our_wallet_pubkey, &opener, &basepoints[LOCAL],
		&basepoints[REMOTE], &tx, &locktime, &tx_blockheight,
		&reasonable_depth, &remote_htlc_sigs, &min_possible_feerate,
		&max_possible_feerate, &possible_remote_per_commitment_point,
		&funding_pubkey[LOCAL], &funding_pubkey[REMOTE],
		&static_remotekey_start[LOCAL], &static_remotekey_start[REMOTE],
	...
	new_tracked_output(&outs, &funding,
			   ...
			   FUNDING_TRANSACTION, funding_sats, FUNDING_OUTPUT,
			   NULL, NULL, NULL);

	/* Record funding output spent */
	send_coin_mvt(take(new_coin_channel_close(
	    NULL, &tx->txid, &funding, tx_blockheight, our_msat, funding_sats,
	...
	status_debug(
	    "Remote per-commit point: %s",
	    type_to_string(tmpctx, struct pubkey, &remote_per_commit_point));
	...
	if (is_mutual_close(tx, scriptpubkey[LOCAL], scriptpubkey[REMOTE])) {
		record_mutual_close(tx, scriptpubkey[REMOTE], tx_blockheight);
		...
		if (is_local_commitment(&tx->txid, &our_broadcast_txid))
			handle_our_unilateral(tx, tx_blockheight, basepoints,
					      opener, remote_htlc_sigs, outs);
		/* BOLT #5:
		 *
		 ...
		 */
					     ...
					     &revocation_preimage)) {
			handle_their_cheat(tx, tx_blockheight,
					   &revocation_preimage, basepoints,
					   opener, outs);
			/* BOLT #5:
			 *
			 * There may be more than one valid, *unrevoked*
			 * commitment transaction after a signature has been
			 * received via `commitment_signed` and before the
			 * corresponding `revoke_and_ack`. As such, either
			 * commitment may serve as the *remote node's*
			 * commitment transaction; hence, the local node is
			...
			handle_unknown_commitment(
			    tx, tx_blockheight,
			    possible_remote_per_commitment_point, basepoints,
			    outs);
		}
	}
