...
struct bitcoin_tx *
htlc_tx(const tal_t *ctx, const struct chainparams *chainparams,
		...
		bool option_anchors_zero_fee_htlc_tx)
{
	...
	htlc_wscript = bitcoin_wscript_htlc_tx(
	    tmpctx, to_self_delay, &keyset->self_revocation_key,
...
void htlc_success_tx_add_witness(
    struct bitcoin_tx *htlc_success, const struct abs_locktime *htlc_abstimeout,
    ...
    bool option_anchor_outputs, bool option_anchors_zero_fee_htlc_tx)
{
	...
	wscript = bitcoin_wscript_htlc_receive(
	    htlc_success, htlc_abstimeout, localhtlckey, remotehtlckey, &hash,
	    revocationkey, option_anchor_outputs,
	    option_anchors_zero_fee_htlc_tx);

	witness = bitcoin_witness_htlc_success_tx(htlc_success, localhtlcsig,
						  remotehtlcsig,
...
{
	u8 **witness;
	u8 *wscript = bitcoin_wscript_htlc_offer(
	    htlc_timeout, localhtlckey, remotehtlckey, payment_hash,
	    revocationkey, option_anchor_outputs,
	    option_anchors_zero_fee_htlc_tx);
...
u8 *htlc_offered_wscript(const tal_t *ctx, const struct ripemd160 *ripemd,
			 const struct keyset *keyset,
			 bool option_anchor_outputs,
			 bool option_anchors_zero_fee_htlc_tx)
{
	return bitcoin_wscript_htlc_offer_ripemd160(
	    ctx, &keyset->self_htlc_key, &keyset->other_htlc_key, ripemd,
	    &keyset->self_revocation_key, option_anchor_outputs,
	    option_anchors_zero_fee_htlc_tx);
}

u8 *htlc_received_wscript(const tal_t *ctx, const struct ripemd160 *ripemd,
			  ...
			  bool option_anchors_zero_fee_htlc_tx)
{
	return bitcoin_wscript_htlc_receive_ripemd(
	    ctx, expiry, &keyset->self_htlc_key, &keyset->other_htlc_key,
	    ripemd, &keyset->self_revocation_key, option_anchor_outputs,
	    option_anchors_zero_fee_htlc_tx);
}
