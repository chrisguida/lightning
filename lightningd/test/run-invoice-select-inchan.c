...
struct channel *
any_channel_by_scid(struct lightningd *ld UNNEEDED,
...
struct bolt11 *
bolt11_decode_nosig(const tal_t *ctx UNNEEDED, const char *str UNNEEDED,
		    const struct feature_set *our_features UNNEEDED,
		    const char *description UNNEEDED,
		    const struct chainparams *must_be_chain UNNEEDED,
		    struct sha256 *hash UNNEEDED, const u5 **sig UNNEEDED,
		    bool *have_n UNNEEDED, char **fail UNNEEDED)
{
...
char *bolt11_encode_(
    const tal_t *ctx UNNEEDED, const struct bolt11 *b11 UNNEEDED,
    bool n_field UNNEEDED,
    bool (*sign)(const u5 *u5bytes UNNEEDED, const u8 *hrpu8 UNNEEDED,
...
void channel_cleanup_commands(struct channel *channel UNNEEDED,
...
void channel_fail_forget(struct channel *channel UNNEEDED,
...
void channel_fail_transient(struct channel *channel UNNEEDED,
			    bool disconnect UNNEEDED, const char *fmt UNNEEDED,
			    ...)
{
...
bool channel_type_has(const struct channel_type *type UNNEEDED,
		      int feature UNNEEDED)
{
	fprintf(stderr, "channel_type_has called!\n");
	abort();
}
/* Generated stub for channel_type_name */
const char **channel_type_name(const tal_t *ctx UNNEEDED,
...
u8 *encrypt_tlv_encrypted_data(
    const tal_t *ctx UNNEEDED, const struct privkey *blinding UNNEEDED,
    const struct pubkey *node UNNEEDED,
    const struct tlv_encrypted_data_tlv *tlv UNNEEDED,
    struct privkey *next_blinding UNNEEDED, struct pubkey *node_alias)
...
u8 *featurebits_or(const tal_t *ctx UNNEEDED, const u8 *f1 TAKES UNNEEDED,
		   const u8 *f2 TAKES UNNEEDED)
{
...
struct channel *
find_channel_by_alias(const struct peer *peer UNNEEDED,
...
struct channel *
find_channel_by_scid(const struct peer *peer UNNEEDED,
		     const struct short_channel_id *scid UNNEEDED)
{
...
bool fromwire_hsmd_sign_bolt12_reply(const void *p UNNEEDED,
				     struct bip340sig *sig UNNEEDED)
{
...
bool fromwire_hsmd_sign_invoice_reply(
    const void *p UNNEEDED, secp256k1_ecdsa_recoverable_signature *sig UNNEEDED)
{
	fprintf(stderr, "fromwire_hsmd_sign_invoice_reply called!\n");
	abort();
}
/* Generated stub for fromwire_node_id */
void fromwire_node_id(const u8 **cursor UNNEEDED, size_t *max UNNEEDED,
		      struct node_id *id UNNEEDED)
{
...
u32 get_feerate(const struct fee_states *fee_states UNNEEDED,
		enum side opener UNNEEDED, enum side side UNNEEDED)
{
...
struct amount_msat
htlc_max_possible_send(const struct channel *channel UNNEEDED)
{
	fprintf(stderr, "htlc_max_possible_send called!\n");
	abort();
}
/* Generated stub for htlc_set_fail */
void htlc_set_fail(struct htlc_set *set UNNEEDED,
		   const u8 *failmsg TAKES UNNEEDED)
{
	fprintf(stderr, "htlc_set_fail called!\n");
	abort();
}
/* Generated stub for htlc_set_fulfill */
void htlc_set_fulfill(struct htlc_set *set UNNEEDED,
...
struct tlv_invoice *invoice_decode_nosig(
    const tal_t *ctx UNNEEDED, const char *b12 UNNEEDED, size_t b12len UNNEEDED,
    const struct feature_set *our_features UNNEEDED,
    const struct chainparams *must_be_chain UNNEEDED, char **fail UNNEEDED)
{
...
char *invoice_encode(const tal_t *ctx UNNEEDED,
		     const struct tlv_invoice *bolt12_tlv UNNEEDED)
{
	fprintf(stderr, "invoice_encode called!\n");
	abort();
}
/* Generated stub for invoice_offer_id */
void invoice_offer_id(const struct tlv_invoice *invoice UNNEEDED,
		      struct sha256 *id UNNEEDED)
{
...
bool invoices_create(struct invoices *invoices UNNEEDED, u64 *inv_dbid UNNEEDED,
		     const struct amount_msat *msat TAKES UNNEEDED,
		     const struct json_escape *label TAKES UNNEEDED,
		     u64 expiry UNNEEDED, const char *b11enc UNNEEDED,
		     const char *description UNNEEDED,
...
void json_add_invstring(struct json_stream *result UNNEEDED,
			const char *invstring UNNEEDED)
{
...
void json_add_stringn(struct json_stream *result UNNEEDED,
		      const char *fieldname UNNEEDED,
		      const char *value TAKES UNNEEDED,
		      size_t value_len UNNEEDED)
{
...
const char *json_scan(const tal_t *ctx UNNEEDED, const char *buffer UNNEEDED,
		      const jsmntok_t *tok UNNEEDED, const char *guide UNNEEDED,
		      ...)
{
...
enum address_parse_result json_to_address_scriptpubkey(
    const tal_t *ctx UNNEEDED, const struct chainparams *chainparams UNNEEDED,
...
struct jsonrpc_request *jsonrpc_request_start_(
    ...
    const char *id_prefix TAKES UNNEEDED, bool id_as_string UNNEEDED,
    ...
    void (*notify_cb)(const char *buffer UNNEEDED,
		      ...
		      const jsmntok_t *paramtoks UNNEEDED, void *) UNNEEDED,
    void (*response_cb)(const char *buffer UNNEEDED,
...
void merkle_tlv(const struct tlv_field *fields UNNEEDED,
		struct sha256 *merkle UNNEEDED)
{
...
int node_id_cmp(const struct node_id *a UNNEEDED,
		const struct node_id *b UNNEEDED)
{
...
struct command_result *param_array(struct command *cmd UNNEEDED,
...
struct command_result *param_index(struct command *cmd UNNEEDED,
				   const char *name UNNEEDED,
				   const char *buffer UNNEEDED,
				   const jsmntok_t *tok UNNEEDED,
...
struct command_result *param_label(struct command *cmd UNNEEDED,
				   const char *name UNNEEDED,
				   const char *buffer UNNEEDED,
				   const jsmntok_t *tok UNNEEDED,
...
struct channel *
peer_any_channel(struct peer *peer UNNEEDED,
...
bool pubkey_from_node_id(struct pubkey *key UNNEEDED,
			 const struct node_id *id UNNEEDED)
{
...
void towire_channel_id(u8 **pptr UNNEEDED,
...
u8 *towire_hsmd_preapprove_invoice(const tal_t *ctx UNNEEDED,
				   const wirestring *invstring UNNEEDED)
{
	fprintf(stderr, "towire_hsmd_preapprove_invoice called!\n");
	abort();
}
/* Generated stub for towire_hsmd_preapprove_keysend */
u8 *towire_hsmd_preapprove_keysend(const tal_t *ctx UNNEEDED,
...
u8 *towire_hsmd_sign_bolt12(const tal_t *ctx UNNEEDED,
			    const wirestring *messagename UNNEEDED,
			    const wirestring *fieldname UNNEEDED,
			    const struct sha256 *merkleroot UNNEEDED,
			    const u8 *publictweak UNNEEDED)
{
...
u8 *towire_hsmd_sign_invoice(const tal_t *ctx UNNEEDED,
			     const u8 *u5bytes UNNEEDED, const u8 *hrp UNNEEDED)
{
...
void wallet_channel_save(struct wallet *w UNNEEDED,
...
void wallet_channel_stats_load(struct wallet *w UNNEEDED, u64 cdbid UNNEEDED,
			       struct channel_stats *stats UNNEEDED)
{
	fprintf(stderr, "wallet_channel_stats_load called!\n");
	abort();
}
/* Generated stub for wallet_channeltxs_add */
void wallet_channeltxs_add(struct wallet *w UNNEEDED,
			   struct channel *chan UNNEEDED,
			   const int type UNNEEDED,
			   const struct bitcoin_txid *txid UNNEEDED,
			   const u32 input_num UNNEEDED,
			   const u32 blockheight UNNEEDED)
{
...
void wallet_delete_peer_if_unused(struct wallet *w UNNEEDED,
				  u64 peer_dbid UNNEEDED)
{
	fprintf(stderr, "wallet_delete_peer_if_unused called!\n");
	abort();
}
/* Generated stub for wallet_extract_owned_outputs */
int wallet_extract_owned_outputs(struct wallet *w UNNEEDED,
				 const struct wally_tx *tx UNNEEDED,
...
bool wallet_htlcs_load_out_for_channel(
    struct wallet *wallet UNNEEDED, struct channel *chan UNNEEDED,
...
char *wallet_offer_find(const tal_t *ctx UNNEEDED, struct wallet *w UNNEEDED,
...
void wallet_transaction_add(struct wallet *w UNNEEDED,
			    const struct wally_tx *tx UNNEEDED,
			    const u32 blockheight UNNEEDED,
			    const u32 txindex UNNEEDED)
{
	fprintf(stderr, "wallet_transaction_add called!\n");
	abort();
}
/* Generated stub for wallet_transaction_height */
u32 wallet_transaction_height(struct wallet *w UNNEEDED,
			      const struct bitcoin_txid *txid UNNEEDED)
{
			  ...
			  struct channel *c)
{
	...
	candidate.r->fee_base_msat = candidate.r->fee_proportional_millionths =
	    candidate.r->cltv_expiry_delta = n;
	candidate.c = c;
...
static struct channel *add_peer(struct lightningd *ld, int n,
				enum channel_state state, bool connected)
{
...
/* There *is* padding in this structure, after pubkey and after
 * cltv_expiry_delta. */
STRUCTEQ_DEF(route_info, ALIGNOF(struct short_channel_id) - 1 - sizeof(u16),
	     pubkey, cltv_expiry_delta, short_channel_id, fee_base_msat,
...
int main(int argc, char *argv[])
{
	...
	candidates[2].c->channel_info.their_config.channel_reserve =
	    AMOUNT_SAT(1);
