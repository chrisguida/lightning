 ...
 * @sighash_type - sighash_type to hash for
 * @tapleaf_script - tapscript leaf for the index that's being 'hashed for',
 * NULL if keyspend
 ...
 */
void sign_hash(const struct privkey *p, const struct sha256_double *h,
...
/**
 * bipmusig_inner_pubkey - produce the sorted taproot inner pubkey using sort
 * order.
 ...
 * @secnonce: secret nonce to be generated. MUST NEVER BE MANUALLY COPIED OR
 * PERSISTED!!!
...
/**
 * bipmusig_partial_sigs_combine_verify - combine and verify partial MuSig
 * signatures
...
/**
 * bipmusig_partial_sigs_combine - Same as bipmusig_partial_sigs_combine_verify
 * but no verification. Should only be used on trusted data!
 * @p_sigs: partial signatures to combine and validate
 * @num_signers: number of partial signatures to combine
 * @sig: final BIP340 signature to output
 */
...
void sign_tx_input(const struct bitcoin_tx *tx, unsigned int in,
		   const u8 *subscript, const u8 *witness,
		   const struct privkey *privkey, const struct pubkey *pubkey,
		   enum sighash_type sighash_type,
		   struct bitcoin_signature *sig);

/**
 * sign_tx_taproot_input - produce a bitcoin signature for a taproot transaction
 * input
 * @tx: the bitcoin transaction we're signing.
